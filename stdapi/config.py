...
#: Seconds in a day, for the settings expressing a duration in days.
_DAY_SECONDS = 86400
...
class _Settings(BaseSettings):
    ...
    realtime_webrtc_allow_private_candidates: bool = Field(
        default=False,
        description=(
            "Accept the ICE candidates a WebRTC caller offers on addresses "
            "that are not globally routable: private, shared, loopback and "
            "link-local ones. They are dropped by default, and an offer left "
            "with no candidate at all is refused, so a caller cannot make the "
            "gateway send STUN probes to addresses inside its own network. "
            "Enable it only where callers legitimately share that network, "
            "such as a same-VPC or on-premises deployment. Hostname and mDNS "
            "('.local') candidates are dropped either way: resolving one is "
            "itself a lookup on the deployment's network."
        ),
    )
    ...
    tenant_api_keys: bool = Field(
        ...
        description=(
            ...
            "tenant_key_ssm_parameter_prefix. Only aws_dynamodb_table is "
            "required alongside it: the delivery prefix has a default.\n\n"
    ...
    tenant_key_ssm_parameter_prefix: str = Field(
        ...
        description=(
            ...
            "every tenant's key, so a deployment sharing an account with "
            "another must not keep the default.\n\n"
    ...
    tenant_key_rotation_overlap_seconds: int = Field(
        default=604800,
        ge=0,
        description=(
            ...
            "soon as the new one is stored. Only the last superseded key is "
            "kept, so the grace ends at the next rotation whatever this is set "
            "to, and it must be shorter than tenant_key_rotation_days. A "
            "compromised key is revoked by disabling the tenant, which refuses "
            "both keys within tenant_key_cache_seconds whatever this value.\n\n"
    ...
    def _validate_realtime_webrtc(self) -> None:
        ...
        Raises:
            ValueError: If the feature is enabled without its optional
                ...
                credentials, or if any media setting is set without the
                feature.
        """
        turn_settings = (
        ...
        if not self.realtime_webrtc_enabled:
            if (
                any(turn_settings)
                or self.realtime_webrtc_stun_server
                or self.realtime_webrtc_allow_private_candidates
            ):
                msg = (
                    "realtime_webrtc_stun_server, realtime_webrtc_turn_* and "
                    "realtime_webrtc_allow_private_candidates require "
                    "realtime_webrtc_enabled: without the feature no WebRTC "
                    "call ever reaches them."
                )
                raise ValueError(msg)
            return
    ...
    def _validate_tenant_keys(self) -> None:
        ...
        if not self.aws_dynamodb_table:
            ...
            raise ValueError(msg)
        prefix = self.tenant_key_ssm_parameter_prefix.rstrip("/")
        if not _SSM_PARAMETER_PREFIX_RE(prefix):
            msg = (
                "tenant_key_ssm_parameter_prefix must be an AWS Systems Manager "
                "Parameter Store path starting with '/' and made of letters, "
                "digits, '_', '.' and '-' segments, e.g. "
    ...
    def _validate_tenant_key_store(self) -> None:
        ...
        if key_id is not None and not _KMS_KEY_ID_RE(key_id):
            ...
            raise ValueError(msg)
        rotation_days = self.tenant_key_rotation_days
        if (
            rotation_days is not None
            and self.tenant_key_rotation_overlap_seconds >= rotation_days * _DAY_SECONDS
        ):
            msg = (
                "tenant_key_rotation_overlap_seconds must be shorter than "
                f"tenant_key_rotation_days ({rotation_days * _DAY_SECONDS} "
                "seconds): only the last superseded key is kept, so an overlap "
                "reaching the next rotation retires nothing."
            )
            raise ValueError(msg)
