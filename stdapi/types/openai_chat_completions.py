...
class _RenamesReasoning(BaseModelResponse):
    """Base for a response carrying an optional Deepseek reasoning field.

    Serializes ``reasoning_content`` under the operator-configured field name
    instead of its own.
    """

    # Deepseek Chat Completion API field.
...
class ChoiceDelta(_RenamesReasoning):
...
class ChatCompletionMessage(_RenamesReasoning):
