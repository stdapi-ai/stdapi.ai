...
from asyncio import Queue, QueueFull, create_task, get_running_loop, sleep
...
class WebRTCCallTransport:
    ...
    async def answer(self, offer_sdp: str) -> str:
        ...
        Raises:
            InvalidOfferError: The offer could not be parsed or answered.
        """
    ...
    async def send_event(self, event: JsonMapping) -> None:
        ...
        if kind == "response.output_audio.delta":
            ...
            return
        if kind == "output_audio_buffer.cleared" or (
            kind == "response.done"
