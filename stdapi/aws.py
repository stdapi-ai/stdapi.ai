...
from stdapi.api_errors import ACCESS_DENIED_CODES, DENIED_CALL_KEY, ApiError, iam_action
...
async def tenant_role_credentials(
    ...
    Raises:
        ApiError: The session could not be opened -- trust revoked, ExternalId
            mismatch or role deleted. 403 with a fixed message: the tenant's
            own registration is at fault, never this deployment, so it is not
            an outage (503) and the body names none of the failure's AWS
            details. Chained to nothing so no AWS code or ARN can leak, and
            so the Bedrock region router never reads the AWS STS failure as a
            failed region.
    """
    ...
    try:
        ...
        raise ApiError(TENANT_CREDENTIAL_FAILURE_MESSAGE, status=403) from None
    return credentials
