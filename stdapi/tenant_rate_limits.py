...
async def admit_tenant_request(
    ...
    try:
        ...
        reservation.refused = True
        raise
    # The write may have outlived the ledger it was entered on -- the window
    # rolled over or the limits changed meanwhile -- and what is in flight is
    # counted on the key's current ledger, where the release will look for it.
    current = _current_ledger(ledger)
    reservation.ledger = current
    # No await from here to the take: the checks and the take are one step.
    if tokens_limit is not None and (
        current.projected_tokens() + max(reservation.estimate, 1) > tokens_limit
        or (requests_limit is None and current.inflight_requests >= _INFLIGHT_MAX)
    ):
        _refuse(current, reservation, tokens_limit, "token")
    if requests_limit is not None:
        # The slot was paid for on the ledger it was reserved from, and is
        # spent there: the current one never held it and never granted it.
        if ledger.granted == 0:
            _refuse(current, reservation, requests_limit, "request")
        ledger.granted -= 1
        if (
            ledger is current
            and ledger.granted < ledger.last_batch // 2
            and not ledger.exhausted
            and ledger.failure is None
        ):
            _kick(ledger, ledger.next_batch())
    current.inflight_estimate += reservation.estimate
    current.inflight_requests += 1
    if scope is None:
