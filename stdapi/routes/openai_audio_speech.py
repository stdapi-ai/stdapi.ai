...
@router.post(
    ...
    description=(
        ...
        "requesting speech marks (`SpeechMarkTypes`) returns timing marks as an "
        "`application/x-json-stream` JSON lines stream, which ignores "
        "`response_format` and does not support `stream_format=sse`.\n\n"
        "The maximum `input` length depends on the selected voice and on this "
        "server's configuration; a request that exceeds it is rejected with "
