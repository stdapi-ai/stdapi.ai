...
async def list_chat_completions(
    ...
    log_request_params({"after": after, "model": model, "metadata": metadata})
    ...
    for completion_id in ids:
        try:
            ...
            continue  # Deleted between the session scan and the read.
        completion = ChatCompletion.model_validate(stored["response"])
        if model is not None and completion.model != model:
            continue
        stored_metadata = completion.metadata or {}
        if any(stored_metadata.get(key) != value for key, value in metadata.items()):
            continue
    ...
    return log_response_params(
        ChatCompletionList(
            data=completions,
            has_more=has_more,
            first_id=completions[0].id if completions else "",
            last_id=completions[-1].id if completions else "",
        )
    )
