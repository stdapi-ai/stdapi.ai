...
class _Endpoint:
    ...
    #: Count invocations only on the series that carried a quantity, for an
    #: endpoint whose usage rides on another endpoint's requests.
    requests_follow_quantities: bool = False
...
_ENDPOINTS: Final[dict[str, _Endpoint]] = {
    ...
    "web_search_calls": _Endpoint(
        ...
        operations=(),
        quantities={
            "SearchUnits": "num_requests",
            "GroundingRequests": "num_requests",
            _REQUESTS: "num_model_requests",
        },
        group_keys=frozenset({"model"}),
        requests_follow_quantities=True,
    ),
}
...
def _build_page(
    ...
    *,
    ...
    """
    # Identity of every series that carried a quantity, so an endpoint riding
    # on another's requests counts only the invocations that produced its own.
    quantity_series = {
...
async def _serve(
    ...
    *,
    ...
    series_list: list[Series] = []
    if buckets:
        await _check_budget(metrics, names)
...
async def _check_budget(metrics: Sequence[str], names: Sequence[str]) -> None:
    ...
    is what bounds the cost. It only sees series published in the last two
    weeks, which makes it a floor on the number the read will match and never a
    substitute for the read itself: an empty listing still reads, because a
    model left idle for a fortnight still has usage inside the range.
    ...
    """
    if len(await list_series(metrics, names)) > SETTINGS.usage_api_max_metrics:
        message = (
...
async def organization_costs(
    ...
    series_list: list[Series] = []
    if buckets:
        await _check_budget(("Cost",), names)
        series_list = await read_series(
            metrics=("Cost",),
            dimension_names=names,
            filters=filters,
