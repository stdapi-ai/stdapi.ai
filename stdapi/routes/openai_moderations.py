...
@router.post(
    ...
    description=(
        ...
        "categories.\n\n"
        "**Available models:**\n"
        "- **`amazon.bedrock-runtime-guardrail`** (the server's default "
        "guardrail, or an explicit `<guardrail-id>`, `<guardrail-id>:<version>`, "
        "or guardrail ARN) — text and image inputs. Content filters map to "
        "the OpenAI categories (`HATE`→`hate`, `INSULTS`→`harassment`, "
        "`SEXUAL`→`sexual`, `VIOLENCE`→`violence`, `MISCONDUCT`→`illicit`); "
        ...
        "overall toxicity score and unmapped labels such as profanity.\n\n"
        ...
        "toxicity detection, when none is configured) and "
        "`text-moderation-*` for toxicity detection.\n\n"
