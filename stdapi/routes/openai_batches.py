...
@router.post(
    ...
    description=(
        ...
        "Every line must name the same model. A file breaking one of those "
        "rules is refused by this call, naming the line, and no batch is "
        "created.\n\n"
        ...
        "streaming and `n` above 1 are not available in a batch; an embeddings "
        "request carries one `input` and returns its vector as numbers.\n\n"
        "**Find compatible models:** call `search_models` with "
        "`route=openai_chat_completion&batch=true`, or "
        "`route=openai_embedding&batch=true` for embeddings. That list is best "
        "effort: a model missing from it is still submitted, and only the "
        "backend decides.\n\n"
