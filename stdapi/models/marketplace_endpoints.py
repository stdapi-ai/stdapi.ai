...
#: SageMaker endpoint status that can serve a request, compared case-insensitively.
_IN_SERVICE_STATUS: Final = "inservice"

#: Statuses an endpoint passes through on its own, not worth reporting to the operator.
_TRANSIENT_STATUSES: Final[frozenset[str]] = frozenset(
    {"creating", "updating", "systemupdating", "deleting"}
)
...
def _model_from_endpoint(
    ...
    cannot answer it. A declined endpoint says why, because a deployment that
    publishes nothing otherwise reads as a deployment with nothing to publish.
    ...
    if (lowered := endpoint_status.lower()) != _IN_SERVICE_STATUS:
