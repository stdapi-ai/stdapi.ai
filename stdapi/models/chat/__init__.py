...
def serves_via_mantle(model_id: str) -> bool:
    ...
    Returns:
        ...
        False for a request signed by a tenant's own AWS credential whenever
        bedrock-runtime serves the model under the same identifier: the
    ...
    # bedrock-runtime, so the class has to be chosen on the same rule.
    return tenant_aws_credential() is None or runtime_home(model_id) is None
