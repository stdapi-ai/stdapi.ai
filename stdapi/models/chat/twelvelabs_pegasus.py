...
_STOP_MAP: dict[str, str] = {"stop": "end_turn", "length": "max_tokens"}
...
async def _format_converse_stream(
    raw_stream: AsyncGenerator[_PegasusChunk], prompt_text: str
    ...
    async for chunk in raw_stream:
        if delta := chunk["message"]:
            ...
            yield {
                "contentBlockDelta": {"contentBlockIndex": 0, "delta": {"text": delta}}
            }
        if stop_reason := chunk.get("stopReason"):
            ...
            yield {
                "messageStop": {"stopReason": _STOP_MAP.get(stop_reason, "end_turn")}  # type: ignore[typeddict-item]
            }
    in_tokens = await estimate_token_count(prompt_text) or 0
