...
class ModerationModel(ModerationModelBase):
    ...
    async def _invoke_checks(self, text: str) -> Moderation | None:
        ...
        if sensitive and sensitive.get("truncated"):
            # An unflagged answer over a partial scan reads exactly like a clean
            # one, and the response has no field upstream defines to say so.
            log_error_details(
                "The personal data check scanned only the beginning of an input "
                "this long: entities after that point were not looked for.",
                level="warning",
            )
        return Moderation(
