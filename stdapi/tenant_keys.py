...
accepting the superseded key for ``tenant_key_rotation_overlap_seconds``, so
a client that re-reads the secret on its own schedule is never locked out.
...
class _Entry:
    ...
    Attributes:
        ...
        previous_until: Epoch second the superseded secret stops
            authenticating at; 0 when there is none.
...
def _build_entry(key_id: str, tenant_item: Item, secret_item: Item) -> _Entry:
    ...
    return _Entry(
        ...
        previous_until=float(previous_until),
        fetched=monotonic(),
    )
...
async def _rotate(key_id: str, tenant_item: Item, secret_item: Item) -> None:
    ...
    version, recorded in the table with the superseded hash kept for the
    overlap, promoted to current, and the record's pending marker cleared.
    The table accepts the new key before it becomes current, so a client
    that re-reads the secret is never handed a key the gateway refuses.
    ...
    if not isinstance(token, str):
        ...
        if overlap := int(SETTINGS.tenant_key_rotation_overlap_seconds):
            rotated.update(
                previous_secret_hash=secret_item["secret_hash"],
                previous_salt=secret_item["salt"],
                previous_until=now + overlap,
            )
