...
async def list_series(
    metrics: Sequence[str], dimension_names: Sequence[str]
) -> list[tuple[str, dict[str, str]]]:
    """List the recently published series matching a dimension set.

    Only series published in the last two weeks are listed, which is what
    CloudWatch's own listing returns; it is used to bound a query's cost before
    paying for it, never as the usage figures themselves.
