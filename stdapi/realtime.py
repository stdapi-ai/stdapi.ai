...
class RealtimeSession:
    ...
    async def _report_tool_call(self, call: ToolCall) -> None:
        ...
        """
        if self._suppressed:
            await self._answer_cancelled_call(call)
            return
        ...
        if self._response is not response:
            # Cancelled while that settled: announcing the call now would place
            # it after the "response.done" a client acts on, which leaves the
            # tool unrun and nobody to answer the model.
            await self._answer_cancelled_call(call)
            return
    ...
    async def _answer_cancelled_call(self, call: ToolCall) -> None:
        """Answer a call whose answer was cancelled, without reporting it.

        Unreported, but still answered: a model left waiting for a result never
        speaks again, and the session would run to its own timeout.

        Args:
            call: What the model called, and is owed a result for.
        """
    ...
    async def _report_answer(self, text: str) -> None:
        ...
        if response.cancelled:
            # Cancelled while the item was announced: the answer is over.
            return
