...
A few rates AWS publishes without a model — because one flat rate covers every model that can incur them — are exposed under a stable identifier of their own rather than repeated on each model. The built-in [web search tool](api_openai_responses.md#openai-gpt-web-search) is priced as `amazon.bedrock-web-search`, on the `grounding_requests` dimension, one unit per query; the guardrail policies are priced under [one identifier per policy](api_openai_moderations.md#billing). Ask for these identifiers by name (`model=amazon.bedrock-web-search`): they are rates rather than models you can call, so they are not part of the unfiltered listing.
