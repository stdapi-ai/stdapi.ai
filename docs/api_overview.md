...
|                   | `GET /ollama`                | Answers `Ollama is running`, the base-URL connection test     | [Models →](api_ollama_models.md#base-url-probe) |

Responses stream as newline-delimited JSON, the transport Ollama clients expect. The model management verbs that write to a local model store — `create`, `copy`, `push`, `delete` and the `/api/blobs/{digest}` upload — are refused with `403`, since this deployment stores no models of its own.
