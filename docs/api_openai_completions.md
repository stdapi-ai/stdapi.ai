...
| `echo`                             |   :material-minus-circle:{ .partial role="img" aria-label="Partial" }    | Prepends each choice's own prompt text to its completion (one leading chunk per choice when streamed); a file-only prompt echoes nothing; rejected with `400` on Mantle-served models |
...
**A Mantle-served model takes one text prompt.** The request is converted to a chat payload before it leaves the gateway, and that conversion accepts a single string: a batch of prompts, a file prompt of any scheme (`https://`, `s3://`, `data:`, `file-id:`), `echo`, `logprobs` and `suffix` are each rejected with `400` naming what was refused. `echo` is a deliberate divergence from the Bedrock Converse path below: honoring it there would need the streamed conversion to know `n` before the first chunk, so it stays a documented `400` here instead of a partial implementation. The OpenAI GPT-5.6 family is served from Mantle by default; clearing [`AWS_BEDROCK_MANTLE_PREFERRED_MODELS`](operations_configuration_models.md#bedrock-mantle-preferred-models) moves it to the classic endpoint, where the full prompt surface applies.

**`echo` prepends the prompt, computed locally.** Amazon Bedrock Converse has no equivalent parameter; the gateway derives each choice's prompt from the prompt text it already built the Bedrock message from and prepends it to that choice's completion (one leading chunk per choice, sent before any generated content, when streamed). A file-only prompt has no text block to echo, so it echoes nothing; a text + files collapse request echoes just its text instruction.

**Seven parameters are accepted and ignored on every model.** `best_of`, `frequency_penalty`, `presence_penalty`, `logit_bias`, `logprobs`, `seed` and `suffix` are accepted, so a client that always sends them keeps working; the Amazon Bedrock Converse API has no equivalent for them and the completion is the same without them. `stop`, `n` and `echo` are honored.
