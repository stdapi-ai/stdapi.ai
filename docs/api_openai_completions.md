...
| `echo`                             |   :material-minus-circle:{ .partial role="img" aria-label="Partial" }    | Prepends each choice's own prompt text to its completion (one leading chunk per choice when streamed); a file-only prompt echoes nothing |
