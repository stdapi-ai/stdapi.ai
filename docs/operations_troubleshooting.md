...
??? failure "The OpenAI GPT-5.6 or GPT-6 models cost about 10% more than before, with no configuration change"
    ...
    - Accept the routing and the rate: nothing to do. It is what makes [OpenAI GPT web search](api_openai_responses.md#openai-gpt-web-search) available. The same routing has three other effects: [input token counting](api_openai_responses.md#input-token-counting) answers `400` for these models, a configured Amazon Bedrock Guardrail stops the server at startup (see the entry above), and usage is billed under Bedrock Mantle rather than Bedrock in [cost reporting](operations_cost_management.md), attributed by [project](operations_configuration_aws.md#bedrock-mantle-project) rather than by IAM principal.
    - Prefer the Global rate: `export AWS_BEDROCK_MANTLE_PREFERRED_MODELS=` (empty). Both families return to the classic endpoint, their server tools are refused with a `400`, and token counting works again.
