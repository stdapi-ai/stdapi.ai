...
??? failure "`413 Payload Too Large` — request or file rejected as oversized"
    ...
    - `A part must not exceed … bytes` means one part of an [upload session](api_openai_files.md#uploads-api) carried more than the 64 MiB an Uploads API part may hold. Split the content into more parts; the official OpenAI client already splits a file at exactly that size, so this only appears when parts are assembled by hand.
