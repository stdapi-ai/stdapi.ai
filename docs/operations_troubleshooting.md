...
??? failure "`404 Not Found` — `The model ... does not exist or you do not have access to it`"
    ...
    - Try the name with and without its `anthropic.` / `openai.` prefix: both forms resolve for those two families, so `anthropic.claude-fable-5` and `claude-fable-5` reach the same model, as do `openai.gpt-oss-120b-1:0` and `gpt-oss-120b`. Models from other providers — Cohere's rerank and embed models among them — are served under their Bedrock IDs only, so name the ID or alias it. A model version Bedrock has retired needs a current one, whichever form you use.
