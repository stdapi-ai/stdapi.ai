...
??? failure "`400 context_length_exceeded`, `prompt is too long`, or an answer cut short below its output limit — the context window is full"
    The conversation no longer fits the model's context window. Each API words the refusal as its vendor does: `context_length_exceeded` on Chat Completions (`param: messages`) and Responses (`param: input`), a message stating the tokens requested (`This model's maximum context length is …`) with no code on legacy Completions, and an `invalid_request_error` reading `prompt is too long` on Anthropic Messages. Some models (Amazon Nova, Meta Llama) refuse a streamed request only once the stream has started: the refusal then arrives as the stream's `error` event after a `200`, not as a `400`. The official SDKs raise it as their base error — `openai.APIError`, or `anthropic.APIStatusError` with status `200` — rather than `BadRequestError`, carrying the same code and message: match on `context_length_exceeded` or `prompt is too long` rather than on the exception class. A streamed Responses request then also ends with `response.failed`.
