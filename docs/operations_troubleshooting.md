...
??? failure "Reasoning set to `none` or disabled still returns reasoning, and its tokens are billed"
    The model always reasons, so `reasoning_effort: "none"` (Chat Completions), `reasoning: {"effort": "none"}` (Responses) or `thinking: {"type": "disabled"}` (Messages) is served with its default adaptive mode and a warning in the request log. See [Disabled Thinking Not Honored](api_anthropic_messages.md#extended-thinking) for the models concerned, and the `none` level under [Reasoning Control](api_openai_chat_completions.md#reasoning-control).
