...
| Web search returns nothing, or MCP tools are never called | [Web search and MCP tools](#tools-and-search) |
...
??? failure "A realtime session goes silent after the model calls a function"
    ...
    - **Reply to the `call_id` the call carried**, as `response.function_call_arguments.done` reports it. An unknown one is answered with an `error` event and never reaches the model.
    - **Answer each call once.** The second reply to the same call is refused with an `error`.
