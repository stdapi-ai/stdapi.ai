...
keywords: stdapi.ai troubleshooting, AWS Bedrock errors, Terraform apply failed, 503 ECS service, 401 API key, 403 permission IAM, AccessDeniedException, 404 model not found, model does not exist or you do not have access to it, model_not_found, gpt-4o 404, gpt-4o not found, gpt-3.5-turbo not found, text-embedding-3-small 404, dall-e-3 model not found, OpenAI model name rejected, which models are available, list available models, ThrottlingException Bedrock, S3 bucket region, ElastiCache capacity, VPC endpoint timeout, podman SELinux, ECONNREFUSED IPv6 service discovery, drop_params ValidationException, ECS task unhealthy restart loop, container healthCheck command, 502 Bad Gateway ECS, 504 Gateway Time-out ECS, ECS task restarts under load, ECS task killed OutOfMemoryError, container killed due to memory usage, MemoryUtilization near 100 percent, Essential container in task exited, ECS stopped task reason OutOfMemoryError, task out of memory audio video, increase ECS task memory, Fargate task memory too small, text to speech input too long, speech input limited to 3000 characters, speech input limited to 20000 characters, generative voice long input, StartSpeechSynthesisStream AccessDenied, service_tier ignored, guardrail ignored, model parameters ignored, tool_choice none ignored, tool_choice none returns tool_calls, model calls a tool I removed, removed tool still called, tools changed between turns, mid-conversation tool changes, model alias configuration, MODEL_ALIASES validation error, extra inputs are not permitted alias, 413 attachment too large, a part must not exceed bytes, upload part too large, 413 uploads part rejected, large image rejected, large PDF rejected, attachments larger than not available, attached files are too large per request, attachment total too large, AWS_S3_REGIONAL_BUCKETS attachment staging, web search returns no results, no error when web search permission missing, external_web_access rejected, external_web_access not available with this model, web search allowed_domains rejected, web search user_location rejected, web_search_options not available with this model, web_search_options rejected chat completions, web_search_options 400, vector store search responses, batch result files never expire, batch output_expires_after, batch results file 404 after expiry, bedrock-websearch InvokeSearch AccessDenied, web search stale cached results, mcp_servers ignored, mcp_toolset dropped, MCP tools never run, MCP connector not supported, browser_toolset_20260801 not available, computer_toolset not available for this model, toolset is not available for this model, browser_state tool result refused, Agent Skills container ignored, container skills never loaded, valid Cognito token rejected 401, Cognito JWT unauthorized, aws_cognito_client_ids required, server will not start after enabling Cognito, server will not start after enabling authentication, API key secret is empty, API key secret stored as binary, secret holds no text value, API_KEY_SECRETSMANAGER_KEY does not hold text, plain string Secrets Manager secret, API_KEY_SSM_PARAMETER empty value, AUTHENTICATION_MODE method not enabled, issuer-cognito-idp issuer mismatch, aws.cognito.signin.user.admin scope missing, WWW-Authenticate Bearer challenge, MCP client cannot discover authentication, oauth-protected-resource 404, protected resource does not match expected, OAUTH_RESOURCE_IDENTIFIER invalid, openid-configuration not served, OAUTH_AUTHORIZATION_SERVERS must name the issuer, published issuer does not match user pool, authorization server derived from Cognito, response format srt not available, verbose_json not available transcription, streamed transcription delivers nothing until the end, transcription stream hangs, stream=true events arrive all at once, transcript.text.delta arrives at the end, StartStreamTranscription missing streamed transcription served as job, StartMedicalStreamTranscription AccessDenied, AWS_TRANSCRIBE_STREAM_LANGUAGES, MaxSpeakerLabels streamed transcription not phrase by phrase, timestamp_granularities rejected, 503 transcription not available, transcription bucket missing, 503 url response format not available, image url response bucket missing, audio too long 10 minutes, nova-2-sonic model not found, InvokeModelWithBidirectionalStream AccessDenied, 503 live conversation not available, tenant API key 503, sk-std key rejected, TENANT_API_KEYS, tenant key not in Parameter Store, revoked tenant key still accepted, TENANT_KEY_CACHE_SECONDS, tenant key rotation, rotated tenant key refused, superseded tenant key still accepted, AWSPREVIOUS tenant key, AWSCURRENT tenant key, TENANT_KEY_SECRETSMANAGER_PREFIX, TENANT_KEY_ROTATION_DAYS, TENANT_KEY_ROTATION_OVERLAP_SECONDS, key_generation ignored, tenant key never rotated, secretsmanager PutSecretValue AccessDenied, tenant model 404, models_allow empty, tenant rate limit, 429 rate_limit_error tenant API key, rate_limit_exceeded tenant, requests_per_minute tenant record, tokens_per_minute tenant record, TENANT_RATE_LIMIT_REQUESTS_PER_MINUTE, TENANT_RATE_LIMIT_TOKENS_PER_MINUTE, x-ratelimit-remaining-requests, anthropic-ratelimit-requests-remaining, retry-after tenant, tenant refused below requests_per_minute, rate limit reached before the limit several instances, tenant 429 no tokens billed, new tenant key rate limited after deploy, token limit refuses eight concurrent requests, token limit 64 requests in flight, latency after enabling tenant rate limits, first request of each minute slow, tokens_per_minute exceeded by a concurrent burst, realtime handshake rate limited, websocket close 3000 rate_limit_exceeded, invalid_request_error rate_limit_exceeded realtime, realtime 429 no retry-after, Ollama 429 no x-ratelimit headers, Cohere 429 no rate limit headers, dynamodb UpdateItem AccessDenied, counter permission probed at startup only, tenant rate limiting not available 503, rate limit counter unavailable 529, tenant AWS credentials, TENANT_AWS_CREDENTIALS, aws_role_arn refused 503, tenant role 403, ExternalId mismatch, sts:AssumeRole tenant role AccessDenied, tenant credential could not be used, tenant account does not have access to this model, per-user cost attribution, sts:TagSession AccessDenied, AWS_BEDROCK_USER_ROLE_ARN, all Bedrock spend on one identity, line_item_iam_principal missing, cost allocation tag IAM principal, requests fail after enabling per-user roles, end user role ApplyGuardrail AccessDenied, 403 guardrail per-user cost attribution, end user role missing model ARN form, 503 speech to speech request fails after a few seconds, real-time audio session timeout, AWS_CONNECT_TIMEOUT generative voice, conversation items missing, conversation 404 after 30 days, cannot add items to conversation, bedrock UpdateSession AccessDenied, conversation metadata update 503, streamed response conversation items delayed, batch stays validating for minutes, batch rejected fewer than 100 requests, batch minimum 100 requests per model, minimum number of records per batch inference job quota, batch model marked legacy 30 days, 503 batch creation refused ValidationException, search_models batch false, model missing from batch=true, batch flag absent COST_TRACKING, batch role not configured, 503 batch API disabled, AWS_BEDROCK_BATCH_ROLE_ARN, iam:PassRole bedrock.amazonaws.com, CreateModelInvocationJob AccessDenied, 503 batch creation denied, batch line failed tool use not supported, batch never reports failed, batch status failed never reached, batch create returns 400 instead of a batch, invalid_json_line, duplicate_custom_id, batch errors data line null, batch results out of order, batch output_file_id missing, batch cached_tokens 0, prompt caching ignored in a batch, cache_control dropped batch, prompt_cache_key batch no effect, batch cache_read_input_tokens zero, vector store file stays in_progress, vector store unsupported_file, attaching a PDF fails unsupported_file, search returns nothing after attaching, 503 vector stores disabled, AWS_S3_VECTORS_BUCKET, vector bucket missing, vector bucket wrong region, s3vectors AccessDenied, 503 vector store operation denied, vector store 409 conflict, vs_kb vector store 404, knowledge base vector store not found, AWS_BEDROCK_KNOWLEDGE_BASE_IDS, knowledge base not allowlisted, bedrock GetKnowledgeBase AccessDenied, 503 attaching a file to a knowledge base store, knowledge base multiple data sources, dataSourceId ambiguous, 400 files cannot be attached to this vector store, knowledge base data source is not a custom one, attach refused knowledge base store, cannot delete a file a search returned, kbdoc file cannot be removed, 400 cannot delete knowledge base vector store, chunking_strategy rejected knowledge base, score_threshold rejected knowledge base, file_batches rejected knowledge base, search query too long knowledge base, realtime websocket upgrade 404, WS /v1/realtime not found, POST /v1/realtime/calls 404, realtime calls endpoint not found, realtime WebRTC not supported, realtime SIP not supported, WebRTC transport realtime API, SIP transport realtime API, SDP offer realtime 404, ALB does not carry UDP, WebRTC media ingress NLB, REALTIME_WEBRTC_ENABLED, WebRTC call no audio, SDP answer silent call, iceConnectionState failed gateway, WebRTC call one way audio, REALTIME_WEBRTC_STUN_SERVER private address candidates, TURN relay coturn UDP blocked, WebRTC hangup 404 another instance, call_id sideband 404 instance, WebRTC call drops 8 minutes, invalid_offer failed to parse offer, unsupported_content_type realtime calls JSON, oai-events data channel no events, realtime session closes after 8 minutes, session_expired close code, realtime session closes mid-conversation, realtime session idle timeout, ALBRequestCountPerTarget realtime autoscaling, 403 NoUserAgent_HEADER websocket, WAF blocks websocket client no user agent, ephemeral client secret rejected, ek_ secret invalid on other instance, REALTIME_CLIENT_SECRET_KEY, realtime client secret multi-instance, 503 feature_unavailable AccessDeniedException, IAM permission denied 503 not available on the current server, model access not enabled 503, 403 permission_error end user role, access denied reading the input s3 URI, 400 input_access_denied, s3 input AccessDenied not 503, accepted bucket GetObject denied, Mantle models missing from catalog, bedrock-mantle unreachable, api.aws endpoint blocked, HTTPS_PROXY not honored, proxy environment variables ignored, NO_PROXY api.aws, com.amazonaws bedrock-mantle VPC endpoint, netrc credentials sent to AWS endpoints, bedrock_mantle_regions_without_endpoint, Mantle unreachable in every region, MantleError service is temporarily unavailable, bedrock-mantle Domain name not found, ClientConnectorDNSError bedrock-mantle, NXDOMAIN bedrock-mantle api.aws, region does not offer Bedrock Mantle, AWS_BEDROCK_MANTLE_REGIONS default, Mantle models missing in one region, server_start_time_ms slow startup, startup takes 30 seconds, ECS container metadata endpoint answered after attempts, ECS task metadata slow startup, unreachable region slows startup, AWS_CONNECT_TIMEOUT startup time, abandoned_background_tasks, background work lost on deploy, vector store file stuck in_progress after deploy, cleanup skipped on shutdown, SHUTDOWN_DRAIN_TIMEOUT, container stop timeout SIGKILL, graceful shutdown drain, work abandoned on scale-in, vector store file failed indexing was interrupted, attach the file again to index it, vector store indexing waits its turn, AWS_SQS_VECTOR_STORE_QUEUE_URL, vector store indexing queue, durable vector store indexing, sqs:SendMessage AccessDenied, must be an Amazon SQS queue URL, must name a standard queue, FIFO queue refused indexing, indexing queue has no dead-letter queue, vector store indexing job abandoned, indexing not picked up after deploy, another server finishes the indexing, tenant key indexing not queued, per-user role indexing not queued, tenant vector store file failed after deploy, aws_bedrock_mantle_preferred_models is incompatible with Amazon Bedrock Guardrails, server will not start guardrail Mantle, guardrail preferred models startup error, AWS_BEDROCK_MANTLE_PREFERRED_MODELS empty, GPT-5.6 more expensive, gpt-5.6 price increased 10%, GPT-5.6 billed under Bedrock Mantle, global cross-region discount lost, restore bedrock-runtime routing dual-homed, Marketplace model endpoint missing from model list, AWS_BEDROCK_MARKETPLACE_ENDPOINTS_ENABLED, AWS_BEDROCK_MARKETPLACE_ENDPOINT_REGIONS, marketplace model endpoint not in service, marketplace model endpoint deployment not registered, marketplace model endpoint requests refused, marketplace/model-endpoint ARN missing from policy, foundation-model wildcard denies marketplace endpoint, marketplace model endpoint reports usage but no cost, SageMaker hosting instance-hour billing, no scale to zero marketplace endpoint, two marketplace endpoints same listing one model missing, first SageMaker request takes minutes, SageMaker endpoint cold start slow, scale to zero SageMaker first request, 503 model is starting up and did not become available in time, AWS_SAGEMAKER_WARMUP_TIMEOUT, NoCapacityInvocationFailures alarm missing, SageMaker endpoint never scales up from zero, inference component CurrentCopyCount stays 0, there is not enough hardware resources on the instances for this endpoint, SageMaker AI endpoint missing from model list, AWS_SAGEMAKER_ENDPOINTS, sagemaker:CallWithBearerToken AccessDenied, sagemaker InvokeEndpoint 403 bearer token, SageMaker AI endpoint reports usage but no cost, wildcard model pattern ambiguous, 400 model pattern matches several models released same date, wildcard model pattern skips a listed model, model release date unknown never selected by pattern, glob model name 400, model pattern fewer than three characters, bare asterisk model not accepted, organization usage endpoints 503, /v1/organization/usage 503, /v1/organization/costs 503, usage API not enabled, USAGE_API, GET /v1/usage not served, retired usage endpoint, usage endpoints empty buckets, no usage reported, organization costs empty, per-endpoint usage buckets missing, Operation dimension missing, CLOUDWATCH_METRICS_REGION, usage reports one region only, usage endpoints 403, tenant API key refused usage endpoints, USAGE_API_ADMIN_SCOPES, usage admin scope missing, usage query too large, too many metric series, USAGE_API_MAX_METRICS, USAGE_API_MAX_RANGE_DAYS, usage range too long, bucket_width 1m refused, one-minute buckets 15 days, num_model_requests, Ollama client shows no models, Ollama connection empty model list, ollama list size 0, ollama pull does nothing, Ollama 404 llama3.2, Ollama tokens per second 0, eval_duration missing, load_duration missing, Ollama is running not returned, Ollama is running base URL, Ollama test connection fails, /api/version probe, Ollama thinking empty, Ollama 401 API key, Unable to locate credentials docker run, ~/.aws mount permission denied container, container runs as uid 65532, nonroot 65532 cannot read credentials, docker run --user HOME /home/nonroot, userns keep-id uid 65532, marketplace image no shell, community image has a shell, docker run sh not found image, entrypoint python command override ignored, bedrock_regions_missing_iam_permission, region reported unreachable but permission denied, missing IAM permission named in startup log, bedrock:ListProvisionedModelThroughputs AccessDenied, models missing in one region only, 403 Forbidden served by the load balancer, 403 not from stdapi.ai, AWSManagedRulesAnonymousIpList, alb_waf_block_anonymous_ips, WAF blocks VPN, WAF blocks CloudShell, WAF blocks cloud host, anonymous IP list 403, terraform apply hangs aws_acm_certificate_validation, ACM DNS validation stuck, PENDING_VALIDATION certificate, no matching Route 53 Zone found, alb_route53_zone_name, hosted zone not found terraform, private hosted zone refused, Anthropic use case form, PutUseCaseForModelAccess, Bedrock model access, Claude model missing from catalogue, unentitled unavailable_models, Bedrock model catalog use case details, aws-marketplace:Subscribe first use, realtime session silent after function call, function_call_output no answer, realtime tools ignored, realtime tool_choice, realtime session.update tools refused, moderation flagged true no category, moderation flagged with every category false, moderations flags harmless text, prompt attack detection moderations, PII detection moderations, AWS_BEDROCK_GUARDRAIL_CHECKS_PII_ENTITIES, AWS_BEDROCK_GUARDRAIL_CHECKS_PROMPT_ATTACK, guardrail checks billed per check, AWS_BEDROCK_GUARDRAIL_SCOPE_TURNS, guardrail no longer flags conversation history, guardrail misses earlier turns, guardrail stopped checking an image, tenant rate limit slow requests, rate-limited tenant latency on every request, requests_per_minute below 16 table write, tool_choice required 400 Claude Opus 5.5, forced tool use refused, reasoning_effort none still reasons, disabled thinking still returns thinking, reasoning cannot be disabled Opus 5.5, find your symptom troubleshooting index, gpt-6 web_search 400, GPT-6 billed under Bedrock Mantle, gpt-6 price increased 10%, guardrail refused GPT-6 Mantle, gpt-6 code_interpreter refused, Server tool 'web_search' is not supported by this model on this server, GPT-6 on Bedrock Mantle, reasoning effort none still reasons responses, context_management ignored, context_management no effect, applied_edits missing, compact_20260112 400, clear_tool_uses_20250919, clear_thinking_20251015 requires thinking, srt not supported amazon.transcribe-medical, medical transcription language not supported, Specialty only available with stream=true, 503 medical transcription not available, 503 live transcription not available medical, StartMedicalTranscriptionJob not authorized region, context_length_exceeded, Your input exceeds the context window of this model, Input tokens exceed the configured limit, context_length_exceeded chat completions, prompt is too long anthropic messages, input exceeds context window, prompt is too long, streamed response server_error context window, truncation auto, context_management compaction, answer shorter than max_output_tokens, response cut short before max_tokens, incomplete max_output_tokens below the limit, stop_reason model_context_window_exceeded, input length and max_tokens exceed context limit, The requested maximum output tokens leave no room for the input, Forcing tool use (tool_choice or function_call) is not available with this model, forced function_call refused, reasoning_effort ignored Claude 3.7 4.5, no reasoning_content small max_tokens, reasoning off with max_completion_tokens 1024, empty thinking text Claude, thinking display summarized, display omitted default, reasoning summary empty Claude, reasoning_content empty Opus 5, thinking field empty with signature, gpt-oss reasoning cannot be disabled, gpt-6 astra reasoning_effort none, thinking display updates 400, display updates not available, unknown_parameter, Unknown parameter, STRICT_INPUT_VALIDATION 400, Please reduce your prompt; or completion length, legacy completions context length
---
...
## :material-magnify: Find your symptom

Search the page with Ctrl-F for the exact error string, or start from the row that matches what you see.

| You see | Start here |
|---|---|
| `terraform apply` fails with `AccessDenied`, or on `no matching Route 53 Zone found`, or hangs on `aws_acm_certificate_validation` | [Terraform / Deployment](#terraform-deployment) |
| `terraform apply` succeeded, but nothing is reachable | [Terraform / Deployment](#terraform-deployment) · [Nothing answers](#reaching-the-gateway) |
| `Unable to locate credentials`, or a volume mount fails under SELinux | [Local Docker / Podman](#local-docker-podman) |
| `503 Service Unavailable` on `/docs` or any endpoint, `ECONNREFUSED`, a task that never becomes healthy | [Nothing answers](#reaching-the-gateway) |
| `403 Forbidden` with no JSON body — served by the load balancer, not by stdapi.ai | [Nothing answers](#reaching-the-gateway) |
| `502 Bad Gateway` or `504 Gateway Time-out` under load, tasks restarting, ECS stopped reason `OutOfMemoryError` | [Nothing answers](#reaching-the-gateway) |
| `401 Unauthorized` | [Authentication & Identity](#authentication-identity) |
| `403 permission_error` on a model call | [Cost attribution and usage](#cost-and-usage) · [Authentication & Identity](#authentication-identity) |
| `404 Not Found` — `The model ... does not exist or you do not have access to it` | [`404` model not found](#models-and-the-catalogue) |
| A Claude model is missing from the catalogue | [`404` model not found](#models-and-the-catalogue) |
| A Marketplace, SageMaker AI or Bedrock Mantle model is missing, slow or uncosted | [Your own models](#your-own-models) |
| `503 feature_unavailable`, or `AccessDeniedException` in the server log | [Startup and shutdown](#startup-and-shutdown) |
| Startup takes tens of seconds, or a deploy leaves work unfinished | [Startup and shutdown](#startup-and-shutdown) |
| `400 Bad Request` from a model call, a parameter that is silently ignored, or a tool called after the request stopped declaring it | [`400` and `429`](#bedrock-request-errors) |
| `429 Too Many Requests`, `ThrottlingException` | [`400` and `429`](#bedrock-request-errors) |
| `context_management` has no effect, `applied_edits` is missing, or `compact_20260112` is refused | [`400` and `429`](#bedrock-request-errors) |
| Claude thinking blocks or reasoning items come back with empty text | [`400` and `429`](#bedrock-request-errors) |
| `429 rate_limit_error` naming a tenant API key, `x-ratelimit-*` or `anthropic-ratelimit-*` headers, a Realtime session closed with `rate_limit_exceeded` | [Authentication & Identity](#authentication-identity) |
| A moderation result is `flagged` with every category `false` | [`400` and `429`](#bedrock-request-errors) |
| Text-to-speech, transcription, a realtime session or a WebRTC call misbehaves | [Audio, speech and realtime](#audio-speech-and-realtime) |
| A file, batch, vector store or conversation never finishes | [Files, batches, vector stores](#files-batches-and-vector-stores) |
| All spend lands on one identity, or `/v1/organization/usage` answers `503`, `403` or empty buckets | [Cost attribution and usage](#cost-and-usage) |
| Web search returns nothing, MCP tools are never called, a browser or computer toolset is refused, or an Agent Skills `container` changes nothing | [Web search and MCP tools](#tools-and-search) |
| An Ollama client shows no models, `0` tokens per second, or no thinking text | [Ollama clients](#ollama-clients) |
| `503`, `504` or `408` mid-stream, `413 Payload Too Large`, `400 context_length_exceeded` or `prompt is too long`, an answer cut short below its output limit | [Long generations](#long-generations) |

Every response the gateway returns for an AWS failure is listed in [AWS error → HTTP status mapping](#aws-error-http-status-mapping).

---

## :material-cloud-upload: Terraform / Deployment { #terraform-deployment }
...
??? failure "`terraform apply` fails with `no matching Route 53 Zone found`"
    ...
    - Bringing your own certificate and DNS instead: pass `alb_certificate_arn` and leave `alb_domain_name` unset, then point your own record at the ALB. See [Advanced Deployment](operations_deploy_advanced.md).

??? failure "`terraform apply` hangs on `aws_acm_certificate_validation`"
...
## :material-docker: Local Docker / Podman { #local-docker-podman }
...
## :material-api: Runtime / First API call { #runtime-first-api-call }

Grouped by what you see. The two `401` entries live under [Authentication & Identity](#authentication-identity).

### :material-lan-connect: Nothing answers — `503` on `/docs`, `ECONNREFUSED`, unhealthy tasks { #reaching-the-gateway }
...
??? failure "`503 Service Unavailable` — on the /docs page or any endpoint"
    ...
    - If it persists longer than 5 minutes, inspect CloudWatch logs for the ECS task. No log group and no task at all points at the image pull rather than the application — see [`terraform apply` succeeds but nothing is reachable](#terraform-deployment).
...
??? failure "`ECONNREFUSED` — connection refused, but only from some clients"
    ...
    - After switching, extend [`PROXY_TRUSTED_HOSTS`](operations_configuration_server.md#proxy-trusted-hosts) with the IPv4-mapped form of each range (`::ffff:10.0.0.0/112`) — a dual-stack listener reports IPv4 peers in that form, and an untrusted proxy means `X-Forwarded-For` is ignored and the load balancer's own address is logged as the client IP.
...
??? failure "Targets never become healthy after setting `TRUSTED_HOSTS`"
    ...
    - The container's own `HEALTHCHECK` is unaffected: it derives its `Host` header from `TRUSTED_HOSTS`. See [`TRUSTED_HOSTS`](operations_configuration_server.md#trusted-hosts).
...
??? failure "The ECS task never reports healthy, or restarts in a loop"
    ...
    - Do not substitute a `curl` or `urllib` one-liner: it sends an untrusted `Host` and is answered with `400` as soon as [`TRUSTED_HOSTS`](operations_configuration_server.md#trusted-hosts) is set.

??? failure "`502 Bad Gateway` or `504 Gateway Time-out` under load, tasks restarting — the container was OOM-killed"
    Neither status comes from stdapi.ai: the load balancer answers them itself when a request arrives with no healthy target to forward to, because the container behind it was just killed for using too much memory. The Terraform module's default Fargate task (512 MiB) sizes the task for text generation and embeddings, where it holds little more than the request in flight — it is not enough for the paths that hold bytes in memory: audio and video through the ffmpeg pipeline, inline input files up to [`MAX_INPUT_FILE_SIZE`](operations_configuration_server.md#max-input-file-size) each, and [`MAX_CONCURRENT_INPUT_DOWNLOADS`](operations_configuration_server.md#max-concurrent-input-downloads) of them fetched at once.
    ...
    - **Fix**: raise the Terraform module's `memory` variable — `1024` or higher for a deployment that serves audio, video or image generation under real load. Valid `cpu`/`memory` pairs are constrained by Fargate; see [Amazon ECS task CPU and memory](https://docs.aws.amazon.com/AmazonECS/latest/developerguide/fargate-tasks-services.html#fargate-tasks-size). See [Cost-Optimized Deployment](operations_deploy_advanced.md#cost-optimized-deployment) for the same sizing note in context.

??? failure "The API endpoint is `http://`, not `https://`"
    Neither `alb_domain_name` nor `alb_certificate_arn` was set, so the [Terraform module](https://github.com/stdapi-ai/terraform-aws-stdapi-ai) creates no HTTPS listener at all and forwards port 80 straight to the tasks. Everything works, but the API — including every API key sent to it — crosses the network in the clear. Never send an API key to that endpoint.
...
??? failure "`403 Forbidden` served by the load balancer, not by stdapi.ai — a VPN, cloud shell or proxy address"
    ...
    - A `403` with no `User-Agent` header on the request is the *other* WAF rule, `NoUserAgent_HEADER` — see [AWS WAF's `NoUserAgent_HEADER` rule reads like an auth failure](operations_deploy_advanced.md#aws-wafs-nouseragent_header-rule-reads-like-an-auth-failure). A `403` that *does* carry a JSON `permission_error` body is [per-user cost attribution](operations_cost_management.md#per-user-attribution), not the WAF.
...
??? failure "`404 Not Found` — `The model ... does not exist or you do not have access to it`"
    ...
    - **Keep the name your application already sends** by mapping it onto a served model with [`MODEL_ALIASES`](operations_configuration_models.md#model-aliases) — an alias only ever points at a model this deployment serves, so what it resolves to is your choice, not a guess.
...
??? failure "A Claude model is absent from the catalogue and answers `404` — the Anthropic use case form has not been submitted"
    ...
    2. Open any Anthropic model in the **Bedrock console → Model catalog** and submit the use case details, or call [`PutUseCaseForModelAccess`](https://docs.aws.amazon.com/bedrock/latest/APIReference/API_PutUseCaseForModelAccess.html). The form asks for your company name, website, industry and intended users, so no deployment can fill it in on your behalf.
    ...
    - Submit it in **every** account that runs a deployment: entitlement follows the account or Organization, not the region or the gateway.
    - A model listed through [AWS Marketplace](operations_cost_management.md#bedrock-marketplace-model-endpoints) is gated differently: AWS creates the subscription on first invocation, which needs `aws-marketplace:Subscribe` and `aws-marketplace:ViewSubscriptions` on the task role and [`AWS_BEDROCK_MARKETPLACE_AUTO_SUBSCRIBE`](operations_configuration_models.md#bedrock-marketplace-auto-subscribe) left enabled (the default). Without those permissions the first call fails with `AccessDeniedException`.
    - A model that is entitled but still missing is a region problem, not an access one — see the `404` entry above and check `AWS_BEDROCK_REGIONS`.
...
??? failure "`400 Bad Request` — a wildcard model pattern is refused as ambiguous"
    ...
    - See [Model Wildcard Patterns](operations_configuration_models.md#model-wildcard-patterns) for the full resolution rule.
...
??? failure "The model list never changes, or keeps advertising models that are gone"
    ...
    - Look for `Refreshing the model list from AWS Bedrock failed` in the server log. It is logged at `warning`, and at `error` once the list is more than two [`MODEL_CACHE_SECONDS`](operations_configuration_models.md#model-cache-seconds) old — the signal that the list is drifting toward the ceiling.
    ...
    - Beyond [`MODEL_CACHE_MAX_STALE_SECONDS`](operations_configuration_models.md#model-cache-max-stale-seconds) (24 hours by default) requests stop being answered from the frozen list and wait for a refresh instead, so a deployment in this state eventually surfaces the real error to clients as a `503` rather than serving a list it cannot confirm.
...
??? failure "Enabling `MODEL_CACHE_SHARED` did not make startup any faster"
    ...
    - Setting it without [`AWS_DYNAMODB_TABLE`](operations_configuration_storage.md#aws-dynamodb-table) fails startup with a message naming both settings.

??? failure "Startup fails with a validation error naming a model alias"
    An alias in [`MODEL_ALIASES`](operations_configuration_models.md#model-aliases-configuration) maps to an object that is not a valid alias configuration, so the server refuses to start rather than ignore it.

    - `Extra inputs are not permitted` names a field that does not exist — check its spelling against the [alias fields](operations_configuration_models.md#model-aliases-configuration).
...
??? failure "`amazon.nova-2-sonic-v1:0` is missing from `/v1/models` or returns `404`"
    ...
    - Add a Region that offers it to [`AWS_BEDROCK_REGIONS`](operations_configuration_aws.md#aws-bedrock-regions); `us-east-1`, `us-west-2`, `ap-northeast-1` and `eu-north-1` did at the time of writing, while `eu-west-3` and `eu-central-1` did not.
...
### :material-package-variant: Your own models — Marketplace, SageMaker AI and Bedrock Mantle { #your-own-models }
...
??? failure "A deployed Marketplace model endpoint does not appear in the model list"
    ...
    - **The feature is off.** [`AWS_BEDROCK_MARKETPLACE_ENDPOINTS_ENABLED`](operations_configuration_models.md#bedrock-marketplace-endpoints-enabled) defaults to `false`.
    - **The endpoint is in a region the server does not serve.** It must be in [`AWS_BEDROCK_REGIONS`](operations_configuration_aws.md#aws-bedrock-regions), and in [`AWS_BEDROCK_MARKETPLACE_ENDPOINT_REGIONS`](operations_configuration_models.md#bedrock-marketplace-endpoint-regions) too when that is set. A Marketplace model endpoint has no cross-region form, so the server cannot reach one outside the regions it is configured to call.
...
??? failure "The first request to a SageMaker AI model takes minutes"
    ...
    - Bound it with [`AWS_SAGEMAKER_WARMUP_TIMEOUT`](operations_configuration_models.md#aws-sagemaker-warmup-timeout), or set it to `0` to fail fast instead of waiting.
...
??? failure "Every request to a SageMaker AI model answers 503 after several minutes"
    The endpoint never came back up, and the server waited its whole [warm-up budget](operations_configuration_models.md#aws-sagemaker-warmup-timeout) before saying so. The message to the caller is deliberately generic; the server log carries the real cause at `WARNING`, naming the endpoint and the elapsed budget.
...
??? failure "A declared SageMaker AI endpoint does not appear in the model list"
    ...
    - **The declaration is malformed.** [`AWS_SAGEMAKER_ENDPOINTS`](operations_configuration_models.md#aws-sagemaker-endpoints) is a JSON object and an unknown field is refused at startup rather than ignored, so the server would not have started.
...
??? failure "Bedrock Mantle models are missing in one region only — `bedrock_mantle_regions_without_endpoint`"
    ...
    - Remove the region from [`AWS_BEDROCK_MANTLE_REGIONS`](operations_configuration_aws.md#bedrock-mantle-regions), or unset it to fall back to the regions of `AWS_BEDROCK_REGIONS` that offer Mantle. Classic Bedrock in that region is unaffected either way.
    - If no configured region offers Mantle, the log says so as well and no Mantle model is served — set [`AWS_BEDROCK_MANTLE_ENABLED`](operations_configuration_aws.md#bedrock-mantle-enabled) to `false` to stop the warning.
...
### :material-restart: Startup and shutdown — `503 feature_unavailable`, slow starts, lost work { #startup-and-shutdown }
...
??? failure "`503` saying the feature is not available on the current server — IAM permission denied on an AWS call"
    ...
    - **Anthropic models only, and not this `503`:** Claude requires a one-time use case form per AWS account or Organization. Without it the model never enters the served catalogue and naming it answers `404 Not Found` — see [the use case form entry](#models-and-the-catalogue) for the steps.
    - Audio, embeddings, and file features need permissions for the relevant services (Polly, Transcribe, Translate, Comprehend, S3) — see [IAM Permissions](operations_iam_permissions.md) for the full IAM reference.
    ...
    - A `400 invalid_request_error` naming an `s3://` input also means the opposite: the denial is on the object *the request pointed at*, in one of the external buckets declared in [`AWS_S3_ACCEPTED_BUCKETS`](operations_configuration_storage.md#aws-s3-accepted-buckets). Only the caller can fix that one — a wrong key, or a bucket policy that does not grant this deployment's role `s3:GetObject` on the object. Objects in the deployment's own buckets keep the `503` above.

??? failure "A permission is named against a region — `bedrock_regions_missing_iam_permission`"
    ...
    - Model discovery calls `bedrock:ListFoundationModels`, `bedrock:ListProvisionedModelThroughputs` and `bedrock:ListInferenceProfiles` in every configured region. Only the provisioned-throughput listing degrades; a denial on either of the other two costs the whole region's models. See [IAM Permissions](operations_iam_permissions.md).
    - With [`AWS_BEDROCK_MARKETPLACE_ENDPOINTS_ENABLED`](operations_configuration_models.md#bedrock-marketplace-endpoints-enabled) set, `bedrock:ListMarketplaceModelEndpoints` and `bedrock:GetMarketplaceModelEndpoint` are called too, and a denial is reported the same way under `<region> (Marketplace endpoints)`. It costs only the endpoints of that region, never a foundation model.
...
??? failure "Startup takes tens of seconds — `server_start_time_ms` far above the usual few seconds"
    ...
    - [`AWS_CONNECT_TIMEOUT`](operations_configuration_aws.md#aws-connect-timeout) bounds each connection attempt and the model-catalog fetch that follows it, so lowering it lowers what an unreachable region can cost. It also bounds failover between healthy regions, so keep it above your real inter-region latency.
...
??? failure "A deploy leaves work unfinished — `abandoned_background_tasks` in the `stop` log event"
    ...
    - Raise [`SHUTDOWN_DRAIN_TIMEOUT`](operations_configuration_server.md#shutdown-drain-timeout) **and** the container stop timeout together — the container runtime sends `SIGKILL` a fixed delay after the stop signal (30 seconds by default on Amazon ECS), so raising the wait alone changes nothing past that delay.
...
??? failure "The server refuses to start: `aws_bedrock_mantle_preferred_models is incompatible with Amazon Bedrock Guardrails`"
    A guardrail is configured — [`AWS_BEDROCK_GUARDRAIL_IDENTIFIER`](operations_configuration_bedrock.md#aws-bedrock-guardrail-identifier) or a [`MODEL_ALIASES`](operations_configuration_models.md#model-aliases-configuration) entry carrying one — while [`AWS_BEDROCK_MANTLE_PREFERRED_MODELS`](operations_configuration_models.md#bedrock-mantle-preferred-models) routes models to Amazon Bedrock Mantle, which cannot apply guardrails. Those models would be served unfiltered, so startup stops instead. The setting defaults to `openai.gpt-5.6,openai.gpt-6`, so a guardrailed deployment meets this without having set it.

    - Keep the guardrail: `export AWS_BEDROCK_MANTLE_PREFERRED_MODELS=` (empty). Every dual-homed model, the GPT-5.6 and GPT-6 families included, is then served by the classic endpoint under the guardrail — and `web_search` and `code_interpreter` are refused with a `400` for those families.
...
### :material-alert-circle-outline: `400` and `429` from a model call — Bedrock request errors { #bedrock-request-errors }
...
??? failure "`400 Unknown parameter` — a field is refused under `STRICT_INPUT_VALIDATION`"
    With [`STRICT_INPUT_VALIDATION`](operations_configuration_observability.md#strict-input-validation) on, a field the gateway does not declare is refused with `400`, code `unknown_parameter`, and the field in `error.param` (e.g., `Unknown parameter: 'messages[0].x'.`). On OpenAI-compatible routes this happens only where OpenAI itself refuses the field — a `developer` message, a file or audio content part, a Responses input item — and the places OpenAI ignores one stay ignored.

    - Remove the field from the request, or move it to where the API accepts it.
    - If a client you do not control sends it, turn `STRICT_INPUT_VALIDATION` off so the gateway ignores it.
...
??? failure "`400 Bad Request` — a client's own control flag reaches Bedrock as a model parameter"
    ...
    - Add that name to [`EXTRA_MODEL_PARAMS_DENYLIST`](operations_configuration_models.md#extra-model-params-denylist) — it is merged with the built-in list, and every other extra parameter keeps being forwarded.
    - If no client needs the passthrough, [`EXTRA_MODEL_PARAMS_DROP_ALL`](operations_configuration_models.md#extra-model-params-drop-all) disables it outright. Per-model defaults set through [`DEFAULT_MODEL_PARAMS`](operations_configuration_models.md#default-model-params) are unaffected — only request-supplied extras are dropped.

??? failure "A tool the request no longer declares is called anyway, or `tool_choice: \"none\"` still returns a tool call"
...
??? failure "`400` — `Forcing tool use (tool_choice or function_call) is not available with this model`"
    The model does not let a request force tool use, as the `tool_choice` row of the [Chat Completions compatibility table](api_openai_chat_completions.md#feature-compatibility) notes. The forced forms are `tool_choice: "required"` or a named function on `/v1/chat/completions` and `/v1/responses`, a named legacy `function_call`, and `{"type": "any"}` or `{"type": "tool"}` on `/v1/messages`.

    - Send `tool_choice: "auto"` (`{"type": "auto"}` on `/v1/messages`), or omit it, and name the tool to call in the prompt.
    - Or pick a model that accepts a forced choice, such as Claude Opus 5 or Claude Fable 5.

??? failure "Reasoning set to `none` or disabled still returns reasoning, and its tokens are billed"
    The model always reasons, so `reasoning_effort: "none"` (Chat Completions), `reasoning: {"effort": "none"}` (Responses) or `thinking: {"type": "disabled"}` (Messages) is served at its default reasoning level and a warning is recorded in the request log. Besides the Claude models that always reason, this includes OpenAI GPT-6 Astra and gpt-oss. See [Disabled Thinking Not Honored](api_anthropic_messages.md#extended-thinking) for the models concerned, and the `none` level under [Reasoning Control](api_openai_chat_completions.md#reasoning-control).

    - Lower the depth instead: `reasoning_effort: "low"` on `/v1/chat/completions`, `reasoning: {"effort": "low"}` on `/v1/responses`, `output_config: {"effort": "low"}` on `/v1/messages`.
    - Or pick a model that can disable reasoning, such as Claude Opus 5 or Claude Sonnet 5.

??? failure "`reasoning_effort` returns no reasoning on Claude 3.7 - 4.5"
    These models spend a thinking budget of at least 1,024 tokens out of the output limit, so a limit of 1,024 or less leaves no room for it: the request is served without reasoning and a warning is logged. See [the note under Reasoning Control](api_openai_chat_completions.md#reasoning-control).

    - Raise `max_completion_tokens` (or `max_tokens`, `max_output_tokens` on `/v1/responses`) above 1,024.

??? failure "Thinking blocks or reasoning items come back with empty text on recent Claude models"
    Claude Opus 4.7 and later, Sonnet 5, Fable and Mythos reason without returning the text by default: the thinking block keeps its signature, and the reasoning tokens are billed in `output_tokens`. Ask for a summary to read it — see [Extended Thinking](api_anthropic_messages.md#extended-thinking) and [Extended Reasoning](api_openai_responses.md#extended-reasoning).

    - `/v1/messages`: set `"display": "summarized"` in `thinking`, for example `{"type": "adaptive", "display": "summarized"}`.
    - `/v1/responses`: set `reasoning.summary` to `auto`, `concise` or `detailed`; the text comes back in the reasoning item's `summary`, not `content`.
    - `/v1/chat/completions` has no summary request, so `reasoning_content` stays empty on these models; use one of the two routes above.
    - A `400` on `"display": "updates"`: that beta value is not available; use `summarized` or `omitted`.
...
??? failure "A request's `service_tier`, guardrail or model parameters are ignored"
    ...
    - The model name may be an alias carrying its own configuration — check the entry in [`MODEL_ALIASES`](operations_configuration_models.md#model-aliases-configuration) for that name. Requests naming it get its service tier, guardrail, metadata and model parameters; requests naming the target model directly do not.
    - The reverse also happens: alias and server-wide configuration apply to models served through Amazon Bedrock's Converse and InvokeModel operations. A model served through Amazon Bedrock Mantle applies the request's own values only, so a configured service tier, metadata or model parameters are ignored there by design — see [the scope note](operations_configuration_models.md#model-aliases-configuration).
    - A request value is discarded on purpose when its override setting is disabled: [`AWS_BEDROCK_ALLOW_SERVICE_TIER_OVERRIDE`](operations_configuration_models.md#aws-bedrock-allow-service-tier-override) for the tier, [`AWS_BEDROCK_ALLOW_GUARDRAIL_OVERRIDE`](operations_configuration_bedrock.md#aws-bedrock-allow-guardrail-override) for the guardrail headers.
    - Otherwise the value comes from the server-wide setting for that field — [`DEFAULT_MODEL_SERVICE_TIERS`](operations_configuration_models.md#default-model-service-tiers), [`DEFAULT_MODEL_PARAMS`](operations_configuration_models.md#default-model-params) or [`AWS_BEDROCK_GUARDRAIL_IDENTIFIER`](operations_configuration_bedrock.md#aws-bedrock-guardrail-identifier). The order is always the request, then the alias, then the setting.

??? failure "`context_management` has no effect, `applied_edits` is missing, or `compact_20260112` is refused with `400`"
    [Context editing](api_anthropic_messages.md#context-editing) is applied by Claude models only. On any other model the field is accepted and ignored rather than refused, because clients such as Claude Code send it on every turn with thinking on: the answer is an ordinary `200`, `usage.input_tokens` keeps growing with the conversation, and the response carries no `context_management`.

    - The server log carries a `warning` saying the `context_management` of the request was ignored, correlated with `x-request-id` — the only place this is reported. Send the request to a Claude model to have the edits applied.
    - On a Claude model, `applied_edits` lists only the edits that cleared something; an empty list means the trigger was not reached — the default `clear_tool_uses_20250919` trigger is 100,000 input tokens — or that the tool results were too short to be worth clearing, which the Anthropic API skips too.
    - Batch results never report `applied_edits`.
    - **`400` naming `compact_20260112`**: compaction is not available, and is refused as an unknown edit type. Only `clear_tool_uses_20250919` and `clear_thinking_20251015` are accepted.
    - **`400` saying `clear_thinking_20251015` requires `thinking`, or must be the first edit**: the Anthropic API's own rule — enable `thinking` (or adaptive thinking) and list that edit first.

??? failure "A guardrail stopped flagging content that appears earlier in the conversation, or an image in the latest turn"
    [`AWS_BEDROCK_GUARDRAIL_SCOPE_TURNS`](operations_configuration_bedrock.md#aws-bedrock-guardrail-scope-turns) is set. On the chat routes the guardrail then evaluates the text of that many trailing user turns only — the saving it buys is paid for in detection; see [Evaluation Scope](operations_configuration_bedrock.md#bedrock-guardrails) for exactly what stops being checked.
...
??? failure "`/v1/moderations` returns `flagged: true` with every category `false` and every score `0.0`"
    The result is correct: the classification hit something the OpenAI moderation categories have no field for, so it is reported on the overall `flagged` field alone. Which one depends on the [model that served it](api_openai_moderations.md#model-support).
    ...
    - `amazon.bedrock-runtime-guardrail-checks`: the prompt attack or personal data check, enabled with [`AWS_BEDROCK_GUARDRAIL_CHECKS_PROMPT_ATTACK`](operations_configuration_bedrock.md#aws-bedrock-guardrail-checks-prompt-attack) or [`AWS_BEDROCK_GUARDRAIL_CHECKS_PII_ENTITIES`](operations_configuration_bedrock.md#aws-bedrock-guardrail-checks-pii-entities). Broad PII entity types flag ordinary prose — a city name is an `ADDRESS` — so narrow the list to the types your policy covers. Each enabled check is also billed separately, which is the other symptom it produces.
    - `amazon.comprehend-toxicity`: the overall toxicity score, or a label with no OpenAI counterpart such as profanity.

??? failure "The OpenAI GPT-5.6 or GPT-6 models cost about 10% more than before, with no configuration change"
    They are served by Amazon Bedrock Mantle by default, so that their `web_search` and `code_interpreter` tools work. Mantle has no cross-region inference profiles, so those requests no longer ride the Global profile the classic endpoint uses by default, and pay the In-Region rate — exactly 10% above the Global one, on every token dimension, for every model with a published rate. GPT-6 Sol and Luna have none yet, so their usage is recorded without a cost.

    - Accept the routing and the rate: nothing to do. It is what makes [OpenAI GPT web search](api_openai_responses.md#openai-gpt-web-search) available. The same routing has two other effects: a configured Amazon Bedrock Guardrail stops the server at startup (see the entry above), and usage is billed under Bedrock Mantle rather than Bedrock in [cost reporting](operations_cost_management.md), attributed by [project](operations_configuration_aws.md#bedrock-mantle-project) rather than by IAM principal.
    - Prefer the Global rate: `export AWS_BEDROCK_MANTLE_PREFERRED_MODELS=` (empty). Both families return to the classic endpoint, and their server tools are refused with a `400`.
    - A deployment that had already disabled [`AWS_BEDROCK_CROSS_REGION_INFERENCE_GLOBAL`](operations_configuration_aws.md#cross-region-global) paid the In-Region rate before and pays it now: nothing changed for it.

??? failure "`400` — `web_search` or `code_interpreter` refused for an OpenAI GPT-6 model"
    Amazon Bedrock serves these tools on the Mantle endpoint only. The GPT-6 models are routed there by default, but only where a configured Mantle region lists them, and Mantle offers them in only a few US regions: a deployment none of whose regions does keeps serving them on the classic endpoint, where no search or code interpreter can run.

    - Set [`AWS_BEDROCK_MANTLE_REGIONS`](operations_configuration_aws.md#bedrock-mantle-regions) to your current Mantle regions plus one whose Mantle endpoint lists the model. An explicit value replaces the default (the Mantle-capable subset of `AWS_BEDROCK_REGIONS`) rather than adding to it, so list every region you still want. Every request for the model is then served from that region, which may sit outside your usual geography, at the In-Region rate; other Mantle-served models may be served from it too.
    - Check that [`AWS_BEDROCK_MANTLE_PREFERRED_MODELS`](operations_configuration_models.md#bedrock-mantle-preferred-models) still names `openai.gpt-6`: an explicit value replaces the default.
    - A request authenticated with a [tenant AWS credential](operations_authentication_security.md#tenant-aws-credentials) is always served by the classic endpoint, and a deployment with a guardrail cannot route to Mantle at all.

### :material-waveform: Audio, speech and realtime — transcription, text-to-speech, WebRTC, WebSocket { #audio-speech-and-realtime }
...
??? failure "`400 Bad Request` — text-to-speech rejects a long `input`"
    ...
    - Set [`AWS_S3_BUCKET`](operations_configuration_storage.md#aws-s3-bucket) for the first region of `AWS_BEDROCK_REGIONS`, and an [`AWS_S3_REGIONAL_BUCKETS`](operations_configuration_storage.md#aws-s3-regional-buckets) entry for every other region that may serve speech — the server log names the one that was missing.
...
??? failure "`400 Bad Request` — a transcription is rejected only for one model"
    ...
    - `amazon.transcribe-medical` refuses `srt` and `vtt` (request `verbose_json` for timed segments), any `language` other than US English, and a `Specialty` other than `PRIMARYCARE` without `stream=true` — see [Medical transcription](api_openai_audio_transcriptions.md#medical-transcription).

??? failure "`503 Service Unavailable` — medical (or live) transcription is not available on the current server"
    Every configured region refused `amazon.transcribe-medical`: medical transcription runs in fewer regions than `amazon.transcribe`, and streamed medical transcription in fewer still, so a deployment whose regions all lack it answers `503` while standard transcription works. A streamed request falls back to a non-streamed transcription when no region offers live medical transcription, except with a `Specialty` other than `PRIMARYCARE`, which then reports *live transcription* as not available. The server log names the refused action.

    - Add a region that offers it to [`AWS_BEDROCK_REGIONS`](operations_configuration_aws.md#aws-bedrock-regions), with a co-located bucket for non-streamed requests — see the [supported regions](https://docs.aws.amazon.com/general/latest/gr/transcribe.html).
    - Setting [`AWS_TRANSCRIBE_REGION`](operations_configuration_aws.md#aws-transcribe-region) to such a region also works, but it pins **all** transcription, `amazon.transcribe` included, to that one region with no failover, and needs a bucket there — check it against your data-residency requirements first.
    - Grant the task role the medical actions — `transcribe:StartMedicalTranscriptionJob`, `transcribe:StartMedicalStreamTranscription`, `transcribe:GetMedicalTranscriptionJob`, `transcribe:DeleteMedicalTranscriptionJob` — see [Speech-to-Text](operations_iam_permissions.md#speech-to-text-optional).

??? failure "`400 Bad Request` — every transcription fails after setting an output encryption key"
    The message is Amazon Transcribe's own failure reason for the job, and it names KMS. Only requests that stage audio in a bucket are affected: [`AWS_TRANSCRIBE_OUTPUT_ENCRYPTION_KEY_ARN`](operations_configuration_storage.md#aws-transcribe-output-encryption-key-arn) encrypts the job's output, so streamed transcriptions, which write nothing, keep working — which is what makes the failure look model-specific at first.
    ...
    - With [`AWS_TRANSCRIBE_REGION`](operations_configuration_aws.md#aws-transcribe-region) unset, a job runs in whichever candidate Region has a co-located bucket, so a single-Region key fails as soon as failover moves the job. Use a [multi-Region key](https://docs.aws.amazon.com/kms/latest/developerguide/multi-region-keys-overview.html), or pin the Region.
    - A key policy conditioned on the [encryption context](operations_configuration_storage.md#aws-transcribe-output-encryption-key-arn) must not require `stdapi-ai.user_id`: it is sent only when the request identifies an end user, so requiring it denies every anonymous call.
...
??? failure "A streamed transcription delivers nothing until the recording ends"
    ...
    - **Set [`AWS_TRANSCRIBE_STREAM_LANGUAGES`](operations_configuration_storage.md#aws-transcribe-stream-languages)** to the languages your callers actually send, which gives phrase-by-phrase delivery to requests that name none.
    ...
    - **Check the live-session permission**: when no configured region can open a live session — `transcribe:StartStreamTranscription` (`transcribe:StartMedicalStreamTranscription` for the medical model) is missing, or no region offers streaming — the request is served as a transcription job, which needs a transcription bucket. Grant the [permission](operations_iam_permissions.md#speech-to-text-optional) or add a region that streams.
...
??? failure "`POST /v1/realtime/calls` answers `404`"
    The WebRTC transport is an **operator opt-in**: without [`REALTIME_WEBRTC_ENABLED`](operations_configuration_bedrock.md#realtime-webrtc-enabled) the endpoint answers `404`, and the error message says so. Enabling it is a deployment-architecture decision — a UDP media path straight to the task, a single instance — not a flag to flip casually; see [WebRTC calls terminated by the gateway](api_openai_realtime.md#webrtc-calls) and [the deployment section](operations_deploy_advanced.md#webrtc-and-sip-need-their-own-ingress) first.
    ...
    - **A client that must speak SIP** — a phone line — belongs behind a voice-agent framework that terminates the media itself and reaches this API over the WebSocket; the SIP call verbs (`accept`, `reject`, `refer`) answer `400` on every deployment. See [Put WebRTC or a phone line in front of the gateway](api_openai_realtime.md#put-webrtc-or-a-phone-line-in-front-of-the-gateway).
    - **Do not try to route the media UDP through the deployment's ALB**: its listeners carry HTTP and HTTPS only. The [Terraform module's media mode](operations_deploy_advanced.md#webrtc-and-sip-need-their-own-ingress) gives the task the public UDP path instead.

??? failure "A WebRTC call gets an SDP answer but carries no audio"
    ...
    - **The task has no public UDP path.** The default deployment keeps tasks in private subnets behind an ALB that cannot carry UDP. Enable the module's `realtime_webrtc_media_enabled`, which assigns the task a public IP and opens the UDP port range in both directions — see [the deployment section](operations_deploy_advanced.md#webrtc-and-sip-need-their-own-ingress).
    ...
    - **The answer advertises a private address.** Behind 1:1 NAT (an ECS task with a public IP) the process sees only its private address; without [`REALTIME_WEBRTC_STUN_SERVER`](operations_configuration_bedrock.md#realtime-webrtc-stun-server) the ICE candidates it sends are unreachable. The module's media mode sets it.
    ...
    - **The caller's network blocks UDP.** Corporate networks often do; only a TURN relay you run gets media through — see [`REALTIME_WEBRTC_TURN_SERVER`](operations_configuration_bedrock.md#realtime-webrtc-turn-server).
...
??? failure "`POST /v1/realtime/calls` answers `400 invalid_offer` — no reachable ICE candidate"
    The offer named only addresses the gateway will not probe. It screens the candidates a caller offers before answering, because those addresses are where its own ICE checks go — see [What a WebRTC call demands of the deployment](api_openai_realtime.md#webrtc-calls).

    - **The caller is on the deployment's own network** (same VPC, on-premises, a LAN), so every candidate it has is a private address. Set [`REALTIME_WEBRTC_ALLOW_PRIVATE_CANDIDATES`](operations_configuration_bedrock.md#realtime-webrtc-allow-private-candidates); it is off by default so an untrusted caller cannot aim UDP probes inside the VPC.
...
??? failure "A WebRTC call drops at exactly 8 minutes"
    Expected: the model session behind the call is capped at 480 seconds by Amazon Nova Sonic, and when it ends the peer connection is torn down with it — see [Session Lifecycle and Limits](api_openai_realtime.md#session-lifecycle-and-limits). A phone-length conversation needs the caller to reconnect with a fresh offer; context does not carry over.
...
??? failure "A realtime session goes silent after the model calls a function"
    The model is waiting for the result. A [function call](api_openai_realtime.md#function-calling) suspends the conversation until a `function_call_output` item naming the same `call_id` comes back, and nothing else is spoken in the meantime.
...
??? failure "An ephemeral client secret works on one instance and is rejected on another"
    ...
    - With no [`API_KEY`](operations_configuration_authentication.md#api-key)-family setting configured at all, each instance falls back to a **random signing key generated per process**, so a secret minted by one instance never verifies on another — the symptom is intermittent rejection that tracks which instance the client's connection happened to land on.
    - Set [`REALTIME_CLIENT_SECRET_KEY`](operations_configuration_bedrock.md#realtime-client-secret-key) explicitly to a value shared by every instance; this also covers a deployment with no API key by design (e.g. behind an IP-restricted ALB).
...
### :material-file-multiple: Files, batches, vector stores and conversations { #files-batches-and-vector-stores }
...
??? failure "S3 error on image generation or audio transcription"
    ...
    - **`503` saying transcription is not available on the current server** means no region that can run a transcription has a bucket at all: set [`AWS_S3_BUCKET`](operations_configuration_storage.md#aws-s3-bucket), [`AWS_TRANSCRIBE_S3_BUCKET`](operations_configuration_storage.md#aws-transcribe-s3-bucket) or an [`AWS_S3_REGIONAL_BUCKETS`](operations_configuration_storage.md#aws-s3-regional-buckets) entry for it. The server log names the settings.
    - **`503` saying the `'url'` response format is not available on the current server** means there is no bucket to host the images a `url` response points at: set [`AWS_S3_BUCKET`](operations_configuration_storage.md#aws-s3-bucket), or request `response_format="b64_json"`, which needs no storage. Image requests are refused before anything is generated, so a misconfigured deployment is never billed for an image it cannot serve; with a guardrail configured, the prompt's check runs first and is billed, because a prompt the guardrail blocks is answered as such rather than as a missing bucket.
...
??? failure "A batch is refused, stuck, or its requests fail one by one"
    ...
    - **A model that batches fine is missing from `search_models?batch=true`, or reports `batch: false`**: that flag is a discovery hint published on a best-effort basis and is never used to reject anything — submit the batch and let the answer decide. It is reported for no model at all while [`COST_TRACKING`](operations_configuration_observability.md#cost-tracking) is disabled, and for a few seconds after startup while the catalogue is still being built.
    ...
    - **`503` on creation, after the endpoints answered normally**: the task role is missing `bedrock:CreateModelInvocationJob` or the `iam:PassRole` statement on the batch service role; the server log names which. A batch that starts and then fails without results usually means the service role itself cannot read or write the bucket under [`AWS_S3_BATCHES_PREFIX`](operations_configuration_storage.md#aws-s3-batches-prefix) — the reason the backend gives is logged as a warning when a job reports `Failed`.
...
??? failure "A vector store file stays in progress, fails, or returns nothing"
    ...
    - **`503` on every vector store endpoint**: the deployment declares no vector storage. Set [`AWS_S3_VECTORS_BUCKET`](operations_configuration_storage.md#aws-s3-vectors-bucket) and [`AWS_S3_VECTORS_REGION`](operations_configuration_storage.md#aws-s3-vectors-region), keep [`AWS_S3_BUCKET`](operations_configuration_storage.md#aws-s3-bucket) set, and grant the [Vector Stores IAM permissions](operations_iam_permissions.md#vector-stores-optional).
    ...
    - **Creating a store fails**: the vector bucket must already exist, in the Region named by [`AWS_S3_VECTORS_REGION`](operations_configuration_storage.md#aws-s3-vectors-region). A vector bucket is a Region-local resource with no failover, so a bucket in another Region is not reachable at all.
...
??? failure "Vector store indexing is not picked up by the queue, or a queued file never settles"
    Only deployments that set [`AWS_SQS_VECTOR_STORE_QUEUE_URL`](operations_configuration_storage.md#aws-sqs-vector-store-queue-url) hand indexing to a queue; without it, indexing runs in the server that accepted the request and everything below is expected behaviour rather than a fault.

    - **The server refuses to start, naming the setting**: the URL is not an Amazon SQS queue URL (`https://sqs.<region>.amazonaws.com/<account-id>/<queue-name>`), it names a FIFO queue, or [`AWS_S3_VECTORS_BUCKET`](operations_configuration_storage.md#aws-s3-vectors-bucket) is unset. FIFO is refused because its deduplication would silently drop a legitimate re-attach of the same files.
    ...
    - **Files attached with a tenant key, or by an identified end user, never reach the queue**: a queued job runs under the server's own identity, so a request whose embeddings are billed to a [tenant AWS credential](operations_authentication_security.md#tenant-aws-credentials) or attributed to an end user under [`AWS_BEDROCK_USER_ROLE_ARN`](operations_configuration_bedrock.md#aws-bedrock-user-role-arn) is indexed in the server that accepted it, keeping the spend where the request put it. Those files settle as `failed` when that server stops first; attach them again.
...
### :material-currency-usd: Cost attribution and the usage endpoints { #cost-and-usage }
...
??? failure "Every request fails after enabling per-user cost attribution"
    ...
    - **`403` once a guardrail is configured**: a guardrail applied during an invocation — [`AWS_BEDROCK_GUARDRAIL_IDENTIFIER`](operations_configuration_bedrock.md#aws-bedrock-guardrail-identifier), a model alias carrying one, or a request-level `moderation` parameter — is evaluated as part of the call the end user signed, so the end user role needs `bedrock:ApplyGuardrail` on the guardrail ARN as well.
    - **`400` naming `safety_identifier`**: [`AWS_BEDROCK_USER_ROLE_REQUIRE_IDENTITY`](operations_configuration_bedrock.md#aws-bedrock-user-role-require-identity) is enabled and the client sends no end user identifier. Either have the client send one, or disable that setting.

??? failure "The organization usage endpoints answer `503 feature_unavailable`"
    [`USAGE_API`](operations_configuration_observability.md#usage-api) is off, which is the default — the endpoints exist and refuse, so this is neither a wrong path nor a rejected credential.

    - Set `USAGE_API=true` and restart — together with [`CLOUDWATCH_METRICS`](operations_configuration_observability.md#cloudwatch-metrics), which publishes the metrics these endpoints are answered from: enabling `USAGE_API` without it fails startup rather than serving endpoints that could never answer. `/v1/organization/costs` additionally needs [`COST_TRACKING`](operations_configuration_observability.md#cost-tracking), which is only a startup warning when missing. The server log names whichever one is missing.
    ...
    - A `404` instead means the prefix is wrong: the routes live under `${OPENAI_ROUTES_PREFIX}/v1/organization/...` — see [`OPENAI_ROUTES_PREFIX`](operations_configuration_server.md#openai-routes-prefix).
...
??? failure "The usage endpoints answer, but every bucket is empty"
    ...
    - **`/v1/organization/costs` alone is empty**: [`COST_TRACKING`](operations_configuration_observability.md#cost-tracking) is disabled, so no cost is computed and no `Cost` metric is published. The usage endpoints are unaffected — [`CLOUDWATCH_METRICS`](operations_configuration_observability.md#cloudwatch-metrics) cannot be disabled while these endpoints answer at all, since `USAGE_API` now refuses to start without it.
    - **The range predates the feature**: usage exists only from the moment `CLOUDWATCH_METRICS` was enabled, and nothing is backfilled. Per-endpoint buckets start later still — only from the moment [`USAGE_API`](operations_configuration_observability.md#usage-api) was enabled, since that is what publishes the `Operation` dimension, so a query grouped or filtered by endpoint is empty over traffic served before it.
    - **A multi-region deployment reports one region's traffic**: the endpoints read a single Amazon CloudWatch region, [`CLOUDWATCH_METRICS_REGION`](operations_configuration_observability.md#cloudwatch-metrics-region), defaulting to the first entry of [`AWS_BEDROCK_REGIONS`](operations_configuration_aws.md#aws-bedrock-regions). Point it at the region the metrics are actually ingested in.
...
??? failure "The organization usage endpoints answer `403`"
    ...
    - **A tenant API key is never accepted**, whatever its scope: these endpoints report the whole deployment's consumption and spend, not one tenant's. Read them with the deployment's own [API key](operations_configuration_authentication.md#api-key).
    - **A user pool token must carry every scope** named in [`USAGE_API_ADMIN_SCOPES`](operations_configuration_observability.md#usage-api-admin-scopes) — all of them, not one of them.
...
??? failure "A usage query is refused as too large, or `bucket_width=1m` is refused over an older range"
    ...
    - **Too many metric series**: the query matched more than [`USAGE_API_MAX_METRICS`](operations_configuration_observability.md#usage-api-max-metrics) (500 by default, which is also CloudWatch's own per-request maximum). Narrow it with `models`, or ask for fewer `group_by` keys.
    - **The range is too long**: it spans more than [`USAGE_API_MAX_RANGE_DAYS`](operations_configuration_observability.md#usage-api-max-range-days) (92 days by default). Split it across several queries, or raise the setting.
...
### :material-tools: Web search and MCP tools { #tools-and-search }
...
??? failure "Web search returns nothing, stale results, or is rejected"
    ...
    - **`400` on `external_web_access`**: the request asked for a value the server does not allow. By default searches stay inside the AWS boundary; set [`AWS_BEDROCK_EXTERNAL_WEB_ACCESS`](operations_configuration_models.md#bedrock-external-web-access) to change what the server does, or [`AWS_BEDROCK_ALLOW_EXTERNAL_WEB_ACCESS_OVERRIDE`](operations_configuration_models.md#bedrock-allow-external-web-access-override) to let requests choose. It travels as an extra model parameter (a top-level request field), not as a field of the tool: a client that sets it on the tool changes nothing.
    ...
    - **`400` saying `web_search_options` is not available with this model**: the [Chat Completions](api_openai_chat_completions.md#web-grounding) parameter reaches the model's own search, and this model runs none — answering it without searching would return something other than what was asked. Send it to an Amazon Nova model that supports web grounding, or ask for the search with the `web_search` tool of [`/v1/responses`](api_openai_responses.md#openai-gpt-web-search).
...
??? failure "A browser or computer toolset is refused, or an Agent Skills `container` changes nothing"
    ...
    - **`400 The 'browser_toolset_20260801' toolset is not available for this model.`** (`computer_toolset_*` reads the same): the [toolsets](api_anthropic_messages.md#browser-and-computer-toolsets) run only on [Bedrock Mantle](api_anthropic_messages.md#bedrock-mantle)-served Claude models, which forward them to the upstream Messages API. A Converse-served model, and a Mantle model whose request is converted to an OpenAI shape, refuse them rather than answer without the browser or the desktop. A replayed `browser_state` tool result is refused the same way, on the same paths.
    - **[`AWS_BEDROCK_EXTERNAL_WEB_ACCESS`](operations_configuration_models.md#bedrock-external-web-access) neither enables nor blocks a toolset**: it governs the built-in web search only. Where a Mantle Claude model serves the toolset, turning [`AWS_BEDROCK_MANTLE_ENABLED`](operations_configuration_aws.md#bedrock-mantle-enabled) off is what leaves no path serving it.
    - **`container` is accepted and the skills are never loaded**: both the identifier string and the `{id, skills}` object [Agent Skills](api_anthropic_messages.md#limits-and-behaviour-to-know) requests use validate, and neither reaches any backend — no container is created or returned, and the response answers as if the field had not been sent. Nothing reports it: unlike the MCP connector above, no warning is logged, so an answer that ignored a skill's instructions is the only signal.

### :material-robot-outline: Ollama clients { #ollama-clients }
...
??? failure "An Ollama client shows no models, or refuses to connect at all"
    ...
    - Point the client at the deployment URL followed by [`OLLAMA_ROUTES_PREFIX`](operations_configuration_server.md#ollama-routes-prefix) — `https://your-host/ollama` by default, not `https://your-host/v1`. Set `OLLAMA_ROUTES_PREFIX` to an empty value to mount at the root instead, for a drop-in swap with a stock Ollama host.
    ...
    - A client that probes its base URL for `Ollama is running` is answered at the [routes prefix](api_ollama_models.md#base-url-probe) — `https://your-host/ollama`, with or without a trailing slash — not at `/`, which serves the deployment's own root document. With `OLLAMA_ROUTES_PREFIX` set to an empty value the two collide and the probe is not served at all; have the client probe [`GET /api/version`](api_ollama_models.md) instead.
...
??? failure "An Ollama client shows no thinking text on a reasoning model"
    ...
    - [`CHAT_COMPLETIONS_REASONING_FIELD`](operations_configuration_observability.md#chat-completions-reasoning-field) set to `none` suppresses the reasoning text on every dialect, including this one. Set it back to `reasoning_content` or `reasoning` to have it emitted.
...
### :material-timer-sand: Long generations and oversized requests — `504`, `408`, `413` { #long-generations }
...
??? failure "`503`/`504`/`408` — request times out mid-stream on long generations"
    ...
    - **`503` from the gateway**: Check [`AI_RESPONSE_TIMEOUT`](operations_configuration_server.md#ai-response-timeout) — it closes stalled upstream model connections; raise it for workloads with long-running generations. The request is **not** retried in another region: the model already ran and AWS bills it either way, so a failover would pay twice for the same generation.
...
??? failure "`400 context_length_exceeded`, `prompt is too long`, or an answer cut short below its output limit — the context window is full"
    The conversation no longer fits the model's context window. Each API words the refusal as its vendor does: `context_length_exceeded` on Chat Completions (`param: messages`) and Responses (`param: input`), a message beginning `This model's maximum context length`, stating the token counts when the model reported them, with no code on legacy Completions, an `invalid_request_error` reading `prompt is too long` on Anthropic Messages, and `{"error": "Your input exceeds the context window of this model…"}` on the Ollama routes. A streamed request gets the same `400` before its stream starts, which the OpenAI and Anthropic SDKs raise as `BadRequestError` and the Ollama Python library as `ResponseError`; only a streamed Responses request gets the refusal as an `error` event followed by `response.failed`, as the OpenAI API streams it.

    - Responses API only: send `truncation: "auto"` to drop the oldest turns instead, or `context_management` to compact the conversation before it reaches the window — see [Context Window Management](api_openai_responses.md#context-window-management). On Anthropic Messages, [context editing](api_anthropic_messages.md#context-editing) clears old tool results.
    - An input that fits the window alone but not beside the output limit is answered with a shorter output on Responses and Anthropic Messages — see [Context Window Management](api_openai_responses.md#context-window-management) and [Limits and behaviour to know](api_anthropic_messages.md#limits-and-behaviour-to-know) — and refused with `context_length_exceeded` naming the completion on Chat Completions, as OpenAI does. An output limit that fills the window on its own is a plain `400` asking to lower it.
    - Choose a model with a larger context window — [`search_models`](api_search_models.md) lists the available models.
...
??? failure "`413 Payload Too Large` — request or file rejected as oversized"
    ...
    - `Attachments larger than … are not available on the current server` means the attachment is too large to travel inside the request and there is nowhere to stage it: no region able to serve that model has an S3 bucket. Set [`AWS_S3_BUCKET`](operations_configuration_storage.md#aws-s3-bucket) for the first region of `AWS_BEDROCK_REGIONS` and an [`AWS_S3_REGIONAL_BUCKETS`](operations_configuration_storage.md#aws-s3-regional-buckets) entry for every other region that may serve the model — the server log names the one that was missing. The same request succeeds unchanged once a bucket exists.
    ...
    - `A part sent as an inline JSON body must not exceed … bytes` means one part of an [upload session](api_openai_files.md#uploads-api) was sent in the JSON form and carried more than the 64 MiB that form may hold — it is decoded in memory, so it is capped. Send the part as a binary `multipart/form-data` upload, which is streamed and bounded only by S3's 5 GiB per part, or split the content into more parts. The binary form never raises this error; the official OpenAI client uses it and already splits a file at 64 MiB.
    - Check [`MAX_INPUT_FILE_SIZE`](operations_configuration_server.md#max-input-file-size) — it caps the bytes of any single file loaded into memory for model input; disabled by default, so if it's set and the error appears, raise it or reduce the input size.
...
## :material-key-variant: Authentication & Identity { #authentication-identity }
...
??? failure "A valid Amazon Cognito token is rejected with 401"
    ...
    - **Wrong app client**: the token's app client must be listed in [`AWS_COGNITO_CLIENT_IDS`](operations_configuration_authentication.md#aws-cognito-client-ids).
    - **Wrong token**: an identity token is rejected unless [`AWS_COGNITO_ACCEPT_ID_TOKEN`](operations_configuration_authentication.md#aws-cognito-accept-id-token) is enabled. Send the **access** token.
    - **Wrong issuer**: pools on the Essentials and Plus tiers can issue `https://issuer-cognito-idp.<region>.amazonaws.com/...`. Set [`AWS_COGNITO_ISSUER_TYPE`](operations_configuration_authentication.md#aws-cognito-issuer-type) to `updated` for those, and confirm the pool ID matches the pool that minted the token.
    - **Missing scope**: a token obtained by signing in with a username and password carries only `aws.cognito.signin.user.admin`. Clear [`AWS_COGNITO_REQUIRED_SCOPES`](operations_configuration_authentication.md#aws-cognito-required-scopes), or have clients obtain tokens from the pool's OAuth 2.0 token endpoint.
...
??? failure "The server does not start after enabling authentication"
    ...
    - **Missing allowlist**: [`AWS_COGNITO_CLIENT_IDS`](operations_configuration_authentication.md#aws-cognito-client-ids) is required with a pool.
    - **Setting without a pool**: any other `AWS_COGNITO_*` variable requires [`AWS_COGNITO_USER_POOL_ID`](operations_configuration_authentication.md#aws-cognito-user-pool-id).
    - **Mode conflict**: [`AUTHENTICATION_MODE`](operations_configuration_authentication.md#authentication-mode) must not demand a method that is unconfigured, nor ignore one that is configured — use `any` to accept both.
    - **Empty API key**: the SSM parameter or Secrets Manager secret named by [`API_KEY_SSM_PARAMETER`](operations_configuration_authentication.md#api-key-ssm) or [`API_KEY_SECRETSMANAGER_SECRET`](operations_configuration_authentication.md#api-key-secretsmanager-secret) exists but holds an empty value — populate it, or unset the setting to run without an API key deliberately.
    - **API key secret is not text**: the secret named by [`API_KEY_SECRETSMANAGER_SECRET`](operations_configuration_authentication.md#api-key-secretsmanager-secret) holds binary data, or the field selected by [`API_KEY_SECRETSMANAGER_KEY`](operations_configuration_authentication.md#api-key-secretsmanager-key) is not a string. Store the key as a plain string, or as a JSON object whose selected field is a string.
...
??? failure "An MCP client or agent cannot discover how to authenticate"
    ...
    - **Nothing published**: `GET /.well-known/oauth-protected-resource` answering `404` means [`OAUTH_RESOURCE_IDENTIFIER`](operations_configuration_authentication.md#oauth-resource-identifier) is unset. That is the default, not a fault — set it to turn discovery on. With an [`AWS_COGNITO_USER_POOL_ID`](operations_configuration_authentication.md#aws-cognito-user-pool-id) configured it is the only setting needed; otherwise set [`OAUTH_AUTHORIZATION_SERVERS`](operations_configuration_authentication.md#oauth-authorization-servers) too.
    - **Nothing in the challenge**: a `401` carrying a bare `WWW-Authenticate: Bearer` means the same thing. Once configured it also carries `resource_metadata="…"` and, with [`OAUTH_SCOPES_SUPPORTED`](operations_configuration_authentication.md#oauth-scopes-supported), `scope="…"`.
    - **Browser-hosted client**: a client running in a page reads the document cross-origin and needs its origin in [`CORS_ALLOW_ORIGINS`](operations_configuration_server.md#cors-allow-origins).
...
??? failure "A client reports the protected resource does not match the expected URL"
    ...
    - **Scheme**: set [`OAUTH_RESOURCE_IDENTIFIER`](operations_configuration_authentication.md#oauth-resource-identifier) to `https://…` when clients reach the deployment over TLS, even though the container itself listens on plain HTTP behind the load balancer.
...
??? failure "The server does not start after configuring authentication discovery"
    ...
    - **No authorization server**: [`OAUTH_AUTHORIZATION_SERVERS`](operations_configuration_authentication.md#oauth-authorization-servers) is required with [`OAUTH_RESOURCE_IDENTIFIER`](operations_configuration_authentication.md#oauth-resource-identifier) unless [`AWS_COGNITO_USER_POOL_ID`](operations_configuration_authentication.md#aws-cognito-user-pool-id) is set, which supplies the issuer; a document naming none leaves a client unable to obtain a token.
    - **Issuer contradicting the pool**: with a user pool configured, the published issuers must include the pool's own — otherwise clients are sent to an authorization server whose tokens every request refuses. Leave [`OAUTH_AUTHORIZATION_SERVERS`](operations_configuration_authentication.md#oauth-authorization-servers) empty to publish exactly the pool issuer, or list it alongside the others.
    - **Setting without an identifier**: the authorization servers and the scopes describe a document that is not published without [`OAUTH_RESOURCE_IDENTIFIER`](operations_configuration_authentication.md#oauth-resource-identifier).
...
??? failure "A tenant API key is refused with 503"
    ...
    - **Missing or wrong table**: [`AWS_DYNAMODB_TABLE`](operations_configuration_storage.md#aws-dynamodb-table) and [`AWS_DYNAMODB_REGION`](operations_configuration_storage.md#aws-dynamodb-region) must name the table holding the tenant records.
...
??? failure "A tenant API key is refused with `429 rate_limit_error`, or a Realtime session closes with `rate_limit_exceeded`"
    The key reached its [per-tenant limit](operations_authentication_security.md#tenant-rate-limits) — its record's `requests_per_minute` or `tokens_per_minute`, or the deployment's [`TENANT_RATE_LIMIT_REQUESTS_PER_MINUTE`](operations_configuration_authentication.md#tenant-rate-limit-requests-per-minute) / [`TENANT_RATE_LIMIT_TOKENS_PER_MINUTE`](operations_configuration_authentication.md#tenant-rate-limit-tokens-per-minute). The message names the key ID and the limit; the `retry-after` header names the seconds left in the minute, and on OpenAI- and Anthropic-compatible routes the `x-ratelimit-*` or `anthropic-ratelimit-*` headers on every response of that key show how much of the minute is left before the refusal. The Cohere- and Ollama-compatible routes answer their own plain `429` envelope with `retry-after` and no rate-limit headers.

    - **Raise or remove the limit**: the record's value wins over the deployment default, so one tenant can be widened without touching the others; a record change reaches every instance within [`TENANT_KEY_CACHE_SECONDS`](operations_configuration_authentication.md#tenant-key-cache-seconds).
    ...
    - **A Realtime session opened but the next connection is refused**: the session's turns count against the token limit and the open session is never cut; the *next* handshake is what the limit refuses — and on the wire it is not a `429`. The upgrade succeeds (`101`), then the server sends one `error` event of type `invalid_request_error` with code `rate_limit_exceeded` and closes with code `3000`, reason `invalid_request_error.rate_limit_exceeded`. No `retry-after` reaches a WebSocket client, so the client backs off itself; only the server request log shows the `429`. A session opened with an [ephemeral client secret](api_openai_realtime.md#ephemeral-client-secrets) also costs a second request, for the `POST /v1/realtime/client_secrets` that mints it.

??? failure "A rate-limited tenant's requests wait on the shared table"
    Declaring a [per-tenant limit](operations_authentication_security.md#tenant-rate-limits) puts writes to the [shared table](operations_configuration_storage.md#aws-dynamodb-table) on the admission path, and a request finding no slot reserved waits for one. Which requests wait follows the limit that is declared:
    ...
    - **A distant region**: [`AWS_DYNAMODB_REGION`](operations_configuration_storage.md#aws-dynamodb-region) is far from the instances; keep the table in the deployment's own region.
...
??? failure "A newly declared tenant has no key in Parameter Store"
    ...
    - **The feature is off**: [`TENANT_API_KEYS`](operations_configuration_authentication.md#tenant-api-keys) must be `true` on the running service, not only in the table.
...
??? failure "A rotated tenant key is refused, or a tenant key is never rotated"
    ...
    - **Keys are delivered through Parameter Store**: rotation needs the Secrets Manager store — [`TENANT_KEY_SECRETSMANAGER_PREFIX`](operations_configuration_authentication.md#tenant-key-secretsmanager-prefix) on the service, or `tenant_key_rotation_days` / `key_generation` in the Terraform module. A `key_generation` raised without it is reported once in the server log and ignored.
    - **Missing permission**: the task role needs the [rotation permissions](operations_iam_permissions.md#tenant-key-rotation) on the secret prefix, and `kms:GenerateDataKey` and `kms:Decrypt` on the key encrypting the secrets. The refusal names the action in the server log; the key keeps working unchanged.
    - **The rotation is not due**: [`TENANT_KEY_ROTATION_DAYS`](operations_configuration_authentication.md#tenant-key-rotation-days) counts from the mint or the last rotation, and `key_generation` only rotates when it exceeds the generation recorded at the previous rotation — raise it again for another rotation.
...
??? failure "A tenant record declaring aws_role_arn is refused with 503"
    ...
    - **The feature is off**: [`TENANT_AWS_CREDENTIALS`](operations_configuration_authentication.md#tenant-aws-credentials) must be `true`, or the attribute removed.
