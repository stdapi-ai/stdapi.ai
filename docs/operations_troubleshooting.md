...
| `503`, `504` or `408` mid-stream, `413 Payload Too Large` | [Long generations](#long-generations) |
...
??? failure "`400 context_length_exceeded` — `Your input exceeds the context window of this model`"
    The conversation no longer fits the model's context window. On a streamed Responses request the same refusal arrives as an `error` event carrying `context_length_exceeded`, followed by `response.failed`.

    - Send `truncation: "auto"` on the [Responses API](api_openai_responses.md#context-window-management) to drop the oldest turns instead, or `context_management` to compact the conversation before it reaches the window.
    - Some models count `max_output_tokens` against the window too: a large output budget can overflow an input that would otherwise fit. Lower it.
