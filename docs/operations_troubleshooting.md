...
??? failure "`404 Not Found` — `The model ... does not exist or you do not have access to it`"
    The name in the request is not one this deployment serves. Adopting the gateway is two client-side changes — the base URL, then the model — and this is the second one arriving as an error: a name your application hard-codes for another provider (`gpt-4o`, `gpt-3.5-turbo`, `text-embedding-3-small`, `dall-e-3`) resolves only if you map it yourself. Nothing is substituted on your behalf, because a lookalike would serve a different model than the one you asked for.
    ...
    - For Anthropic SDK clients, use either the full Bedrock ID (`anthropic.claude-fable-5`) or the Anthropic alias (`claude-fable-5`) — both resolve automatically. A Claude version Bedrock has retired needs a current one.
...
??? failure "`400` — the `file_search` tool is refused"
    Retrieval over a vector store is not run for the model on this endpoint, and a request whose `file_search` tool were dropped would come back with a confident answer that never consulted the stores it attached.

    - Search the stores yourself with [`POST /v1/vector_stores/{id}/search`](api_openai_vector_stores.md) and pass the passages you get back in the request input.
    - Models served natively by [Bedrock Mantle](features.md#bedrock-mantle-models) receive the tool unchanged and answer it themselves when they support it.
...
??? failure "A vector store file stays in progress, fails, or returns nothing"
    ...
    - **A file settles as `failed` with `unsupported_file`**: its bytes are not text. Only text files can be indexed; convert documents first — [RAG Pipelines](use_cases_rag.md#document-parsing) shows a conversion stage.
    - **A file settles as `failed` with `server_error`**: indexing hit a backend error. Check the `background` log event sharing the request's `id`, then detach the file and attach it again.
    - **A file stays `in_progress` far longer than the others**: a large file is many passages, each embedded in turn. If it never settles, the process that was indexing it was replaced mid-flight — detach the file and attach it again.
