...
??? failure "A deploy leaves work unfinished — `abandoned_background_tasks` in the `stop` log event"
    ...
    - A vector store file left `in_progress` after a deployment, or a temporary object that outlived its request, is what those counts look like from the outside. Re-attach the file, or let the object's lifecycle rule expire it.
