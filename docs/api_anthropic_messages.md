...
| Server tool counters (`usage.server_tool_use`) | :material-close-circle:{ .unsupported role="img" aria-label="Unsupported" } | Always omitted, so `web_search_requests` is never reported — see [Web Grounding](#web-grounding) |
| **Other**                             |                                          |                                                                                              |
...
- **No search counter in `usage`**: `usage.server_tool_use` is always omitted, so `web_search_requests` never carries a number. Count the `server_tool_use` blocks whose `name` is `web_search` instead — each one is a search, answered by a `web_search_tool_result` block carrying the same `tool_use_id`. Filter on the name: a model that also runs code emits `server_tool_use` blocks for that too, so counting them all overstates the searches.
...
- `mcp_servers` and every `mcp_toolset` entry are dropped; no connection is made and none of the server's tools can be called. An `authorization_token` you supply is never forwarded to any backend, and never reaches the logs: not the `request_params` field ([`LOG_REQUEST_PARAMS`](operations_configuration_observability.md#log-request-params)), and not the report of a request rejected as malformed, which [names the fields that failed](operations_logging_monitoring.md#reading-the-logs) and none of their values.
