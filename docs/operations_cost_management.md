...
4. **Built-in Defaults**: A few models are not in the Price List API at all — the Stability AI Image Services, priced on the [AWS pricing page](https://aws.amazon.com/bedrock/pricing/), and OpenAI's hosted GPT models, priced on their Bedrock model cards. A model Bedrock serves before AWS publishes any rate for it — GPT-6 Luna and Sol, GLM 4.6 — is priced at its vendor's published rate, which Bedrock matches for its siblings: OpenAI's Standard rate for Global and its regional-processing premium for In-Region, Z.ai's API rate in the US Regions. Those rates ship built in, used only when AWS publishes no row for the model; `COST_PRICE_OVERRIDES` still takes precedence
