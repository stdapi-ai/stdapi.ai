...
Running stdapi.ai bills into three independent buckets. Only the first usually matters at scale, but the other two decide the floor of what you pay when traffic is low.
...
## :material-server: Gateway Infrastructure Cost

The gateway itself is a container, so its infrastructure cost is small and largely fixed — but it is a floor you pay whether or not any request arrives.

The [Terraform module](operations_getting_started.md) provisions:

| Component                         | Notes                                                                    |
|:----------------------------------|:-------------------------------------------------------------------------|
| **ECS Fargate** service           | The dominant infrastructure cost; scales with `autoscaling_min_capacity`  |
...
- **Trim observability volume.** `LOG_LEVEL`, request/response payload logging and `CLOUDWATCH_METRICS` all affect CloudWatch ingestion charges. See [Controlling Log Verbosity](operations_logging_monitoring.md#controlling-log-verbosity).

!!! note "Estimating before you deploy"
    stdapi.ai publishes no infrastructure estimate: the total depends on your region, container size, replica count and whether the module creates networking. Price the component list above with the [AWS Pricing Calculator](https://calculator.aws/) for your own configuration.

---

## :material-tag-outline: Gateway License Cost
