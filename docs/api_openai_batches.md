...
| `service_tier`                   |   :material-minus-circle:{ .partial role="img" aria-label="Partial" }    | Accepted and ignored — a batch always runs on Amazon Bedrock batch inference, and the completions report no `service_tier` because the OpenAI schema publishes no value for it; the usage is [billed at the batch rate](#pricing) |
