...
!!! warning "The Listing Window"
    ...
    - **A burst of thousands of batches inside a single minute can outrun the
      seek**, widening the window toward the older end of that burst instead
      of reaching the newest batch in it — a rate no normal workload
      sustains, since each batch requires its own inference job and a real
      deployment is bound by Bedrock's batch job quota long before it
      reaches this.
