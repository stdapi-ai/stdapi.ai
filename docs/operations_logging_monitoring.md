...
| `context`                   | str | Present as `"long"` only when the call's prompt (input + cache read/write tokens) exceeded the boundary that model publishes — 200K tokens for most models, 272K for the OpenAI frontier models (GPT-5.4 and later); billed at the [long-context rate](operations_cost_management.md#long-context-pricing) where AWS publishes one |
...
| `input_seconds`             | int | Real input media duration in seconds (Transcribe audio, rounded up to the second; audio/video embedding inputs) |
...
- Rejected requests: A `400` from request validation lists the fields that failed and why, as `<field path>: <message>` entries in `error_detail` — the first 20 distinct faults, then a count of the rest. The values themselves are never logged, so a credential or an attachment a rejected body carries stays out of the log; the caller is answered in the wording of the API it called (see [Using stdapi.ai](api_overview.md#using-stdapiai)).
