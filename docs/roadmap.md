...
| ![Ollama](styles/logo_ollama.svg){: style="height:20px;width:20px"} **Ollama** | [`/ollama/api/tags`, `/ollama/api/show`, `/ollama/api/ps`, `/ollama/api/version` and `/ollama/api/pull`](api_ollama_models.md) – the discovery verbs an Ollama tool calls before anything else. The verbs that write to a local model store — `create`, `copy`, `push`, `delete` and the `/api/blobs/{digest}` upload — are refused, since this deployment stores no models of its own; the base URL answers `Ollama is running` for a client's connection test | ![Amazon Bedrock](styles/logo_amazon_bedrock.svg){: style="height:20px;width:20px"} Amazon Bedrock |
...
- **The documentation pages load a current Swagger UI and ReDoc**: `swagger-ui-dist` 5.33.0, which scopes its HTML sanitiser to itself instead of mutating the page's global one and lays out only the part of a large specification that is on screen, and `redoc` 2.5.4, which carries accessibility fixes and patched bundled dependencies.
