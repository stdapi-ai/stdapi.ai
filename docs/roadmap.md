...
!!! info "New IAM Permissions, All Optional"
    **Nothing new is required.** A deployment that enables none of the features below needs the policy it already has; the Terraform module grants each set when its feature is turned on. See [IAM Permissions](operations_iam_permissions.md) for the statements in full, and note that a denial now [names the action it needs](operations_troubleshooting.md) in the server log rather than leaving you to guess it.
    ...
    - **Your own SageMaker AI endpoints** — `sagemaker:CallWithBearerToken` and `sagemaker:InvokeEndpoint` on the endpoint ARNs you serve, for [Amazon SageMaker AI endpoints](operations_configuration.md#aws-sagemaker-endpoints).
    ...
    - **Tenants** — `ssm:PutParameter` and `ssm:GetParameter` to deliver each tenant its key once, `kms:Encrypt`, `kms:Decrypt` and `kms:GenerateDataKey` on the key named by [`TENANT_KEY_SSM_KMS_KEY_ID`](operations_configuration.md#tenant-key-ssm-kms-key-id) when you set one, and `sts:AssumeRole` on exactly the roles tenants declare when they [bring their own AWS account](operations_authentication_security.md#tenant-aws-credentials).
...
!!! warning "Behavior Changes"
    ...
    - **Mantle routing and the per-user role's identity requirement are refused together.** A Mantle request is signed with the server's own credentials, so [`AWS_BEDROCK_USER_ROLE_REQUIRE_IDENTITY`](operations_configuration.md#aws-bedrock-user-role-require-identity) can never apply to a model routed there: a deployment setting both with `AWS_BEDROCK_MANTLE_PREFERRED_MODELS` non-empty stops at startup, and a Mantle request without an end-user identity answers `400`.
...
| Provider | Endpoint/Feature | AWS Backend |
|---|---|---|
| **stdapi.ai** | [Wildcard model names](operations_configuration.md#model-wildcard-patterns) – any request that names a model may name a pattern instead — `claude-sonnet-*`, say — and the most recently released match serves it; an ambiguous tie is refused rather than guessed | ![Amazon Bedrock](styles/logo_amazon_bedrock.svg){: style="height:20px;width:20px"} Amazon Bedrock |
...
| WebRTC media path provisioned | In media mode the Terraform module provisions the whole UDP path on the VPC it creates — the matching egress, and the application-subnet network ACL entries the media needs; bring your own `subnet_ids` and those entries stay yours to widen |
...
- **A model both catalogues name stays reachable through bedrock-runtime**: publishing the preferred entry dropped the other, so a tenant's own AWS credential — which can neither sign nor pay for a Bedrock Mantle request — was refused outright and the model was reported impossible to batch. The displaced bedrock-runtime entry is kept off-catalogue and read by everything that must reach that endpoint, while the published catalogue keeps naming the preferred entry.
...
- **A streamed Anthropic answer from a Marketplace endpoint always carries its `stop_reason`**, so a client can tell a finished answer from a truncated one.
- **A job-served diarized transcript keeps its own spacing**, instead of gaining a stray space at a silent segment.
