...
**Latest: v1.15.0** – A gateway-wide reliability, quality and performance release: hundreds of fidelity fixes validated against real client applications, measurably lower processing overhead, and existing features completed across every dialect — prompt caching, reasoning controls, speech, Cohere, and guardrails everywhere. See the [full release notes](#v1150-reliability-performance-feature-completeness) below.
...
!!! warning "Behavior Changes"
    ...
    - **The container health probe now respects `TRUSTED_HOSTS`.** The image's `HEALTHCHECK` runs `python3 /usr/local/bin/healthcheck.py`, which requests `/health` with a `Host` header derived from [`TRUSTED_HOSTS`](operations_configuration.md#trusted-hosts) — a correct list keeps the container healthy with no extra entry. Deployments that override the probe in Compose or an ECS task definition should run that same command. Note that a load balancer health check still sends the target's IP address as the `Host` and is rejected with `400` when the allow-list is enabled.
