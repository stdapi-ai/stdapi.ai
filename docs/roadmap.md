...
!!! abstract "At a glance"
    ...
    - **Medical transcription.** [`amazon.transcribe-medical`](api_openai_audio_transcriptions.md#medical-transcription) transcribes clinical dictation and patient–clinician conversations in US English, streamed phrase by phrase, with six medical specialties.
...
!!! info "New IAM Permissions, All Optional"
    **Nothing new is required.** A deployment that does not serve medical transcription needs the policy it already has; the Terraform module grants these wherever it grants transcription. See [Speech-to-Text](operations_iam_permissions.md#speech-to-text-optional).
...
!!! warning "Behavior Changes"
    ...
    - **The OpenAI GPT-6 family is served by Bedrock Mantle by default**, wherever a configured Mantle region lists it, so its `web_search` and `code_interpreter` work with no configuration. [`AWS_BEDROCK_MANTLE_PREFERRED_MODELS`](operations_configuration_models.md#bedrock-mantle-preferred-models) now defaults to `openai.gpt-5.6,openai.gpt-6`. It is the trade GPT-5.6 already made: no Global routing discount, so **GPT-6 Astra costs exactly 10% more per token**, no guardrails, and usage billed and reported under Bedrock Mantle. An empty value keeps both families on the classic endpoint. An explicit value replaces the default, so a deployment that already sets one keeps GPT-6 where it was.
    ...
    - **Validation errors are worded as each vendor words them.** On the OpenAI routes `error.param` names the field in OpenAI's notation (`messages[0].content`) and `error.code` the failure (`unknown_parameter`, `missing_required_parameter`, `invalid_type`, `invalid_value` and the range codes); Anthropic routes answer `<field>: <reason>` with no gateway prefix. A client matching on the old message text needs updating. See [Using stdapi.ai](api_overview.md#using-stdapiai).
    - **[`STRICT_INPUT_VALIDATION`](operations_configuration_observability.md#strict-input-validation) refuses an unknown field only where OpenAI does.** It is now ignored on chat messages other than `developer`, on content parts, tool calls and tool definitions, on Responses function tools and on moderation requests. A replayed tool call carrying an unknown field is now ignored whatever the setting.
    - **Token counting no longer answers `400` for any text model.** Models served by Bedrock Mantle (the GPT-5.6 and GPT-6 families by default), Marketplace and SageMaker AI endpoints, inputs past the context window and requests offering server tools are all counted, and [`search_models`](api_search_models.md) lists both counting routes for every text model. Most of these counts are approximate: see Known Limitations below.
...
!!! note "Known Limitations"
    ...
    - **Recent Claude models return no reasoning text on Chat Completions.** Opus 4.7 and later, Sonnet 5, Fable and Mythos return it only as a summary, which Chat Completions cannot request: use `thinking.display` on [Messages](api_anthropic_messages.md#extended-thinking) or `reasoning.summary` on [Responses](api_openai_responses.md#extended-reasoning).
...
|----------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------|
| ![OpenAI](styles/logo_openai.svg){: style="height:20px;width:20px"} **OpenAI** | [`truncation: "auto"` on Responses](api_openai_responses.md#context-window-management) – an input over the context window has its oldest turns dropped, then the latest turn's longest text cut at its end, until it fits. System and developer messages are always kept, `usage` reports the input that was kept, and `input_tokens` counts what a response would keep | ![Amazon Bedrock](styles/logo_amazon_bedrock.svg){: style="height:20px;width:20px"} Amazon Bedrock - Converse API, Bedrock Mantle |
| ![OpenAI](styles/logo_openai.svg){: style="height:20px;width:20px"} **OpenAI** | [`context_management` compaction on Responses](api_openai_responses.md#context-window-management) – once the input crosses `compact_threshold`, everything but the latest step is summarized into a `compaction` item that leads `output` and stands for the whole history when sent back; streamed at index 0 before the answer. A trailing `compaction_trigger` item compacts on demand, where it was silently dropped. Both calls are billed, and `usage` adds them up | ![Amazon Bedrock](styles/logo_amazon_bedrock.svg){: style="height:20px;width:20px"} Amazon Bedrock - Converse API, Bedrock Mantle |
| ![OpenAI](styles/logo_openai.svg){: style="height:20px;width:20px"} **OpenAI** & ![Anthropic](styles/logo_anthropic.svg){: style="height:20px;width:20px"} **Anthropic** | [The output is capped when only it overflows](api_openai_responses.md#context-window-management) – an input the window holds alone but not beside `max_output_tokens` or `max_tokens` is answered with a shorter output on Responses and [Anthropic Messages](api_anthropic_messages.md#limits-and-behaviour-to-know), as both vendors answer it. Chat Completions keeps refusing it, as OpenAI does | ![Amazon Bedrock](styles/logo_amazon_bedrock.svg){: style="height:20px;width:20px"} Amazon Bedrock |
| ![OpenAI](styles/logo_openai.svg){: style="height:20px;width:20px"} **OpenAI** & ![Anthropic](styles/logo_anthropic.svg){: style="height:20px;width:20px"} **Anthropic** | [Token counting on every text model](api_openai_responses.md#input-token-counting) – `/v1/responses/input_tokens` and `/v1/messages/count_tokens` count an input past the context window, always above the window so a client learns the conversation no longer fits, plus server tools with their replayed calls, results and citations, inline images, and documents and remote files at an allowance | ![Amazon Bedrock](styles/logo_amazon_bedrock.svg){: style="height:20px;width:20px"} Amazon Bedrock |
| ![Anthropic](styles/logo_anthropic.svg){: style="height:20px;width:20px"} **Anthropic** | [Context editing](api_anthropic_messages.md#context-editing) – `clear_tool_uses_20250919` and `clear_thinking_20251015` with every documented option. Claude applies the edits and reports them in `applied_edits`, on the final `message_delta` when streaming, and `count_tokens` counts the edited prompt beside `original_input_tokens`. The beta flag is added for you, so the header is optional | ![Amazon Bedrock](styles/logo_amazon_bedrock.svg){: style="height:20px;width:20px"} Amazon Bedrock - Converse API, Bedrock Mantle |
| ![Anthropic](styles/logo_anthropic.svg){: style="height:20px;width:20px"} **Anthropic** | [`thinking.display`](api_anthropic_messages.md#extended-thinking) – `summarized` or `omitted` reaches Claude as sent, so Opus 4.7 and later, Sonnet 5, Fable and Mythos, which omit their thinking text by default, return a summary when asked. The thinking tokens bill the same either way. On Responses, `reasoning.summary` asks for the same summary | ![Amazon Bedrock](styles/logo_amazon_bedrock.svg){: style="height:20px;width:20px"} Amazon Bedrock - Converse API |
| ![OpenAI](styles/logo_openai.svg){: style="height:20px;width:20px"} **OpenAI** | [Medical transcription](api_openai_audio_transcriptions.md#medical-transcription) – `amazon.transcribe-medical` on `/v1/audio/transcriptions`, for a physician's dictation or a patient–clinician conversation (`Type`), with `Specialty` and medical custom vocabularies. `json`, `text`, `verbose_json` and `diarized_json` are served, and `stream=true` streams phrase by phrase. A client that can only send a model name reaches a fixed variant through a [model alias](operations_configuration_models.md#model-aliases-configuration) | ![Amazon Transcribe](styles/logo_amazon_transcribe.svg){: style="height:20px;width:20px"} Amazon Transcribe Medical |

#### :material-brain: Models & Reasoning

| Provider                                                                        | Endpoint/Feature                                                                                                                      | AWS Backend                                                                                                                 |
|----------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------|
| ![OpenAI](styles/logo_openai.svg){: style="height:20px;width:20px"} **OpenAI** | [GPT-6 web search and code interpreter](api_openai_responses.md#openai-gpt-web-search) – served with no header and no configuration wherever a configured Mantle region lists the model, and Chat Completions reaches GPT-6 natively there rather than converted | ![Amazon Bedrock](styles/logo_amazon_bedrock.svg){: style="height:20px;width:20px"} Amazon Bedrock Mantle |
| ![OpenAI](styles/logo_openai.svg){: style="height:20px;width:20px"} **OpenAI** | [Reasoning effort on GPT-5.x, GPT-6 and gpt-oss](api_openai_chat_completions.md#reasoning-control) – from every dialect: `reasoning_effort`, `reasoning.effort`, `output_config.effort` or `thinking`. `none` turns reasoning off where the model can; GPT-6 Astra, and gpt-oss outside Bedrock Mantle, cannot, and are served at their default level with a warning in the request log. gpt-oss gets its own `low` / `medium` / `high` scale | ![Amazon Bedrock](styles/logo_amazon_bedrock.svg){: style="height:20px;width:20px"} Amazon Bedrock |
| ![Moonshot](styles/logo_moonshot.svg){: style="height:20px;width:20px"} **Moonshot** | [Kimi K3](api_openai_chat_completions.md#prompt-caching) – reasoning effort from every dialect, with `none` answering without reasoning, and automatic prompt caching: a repeated prefix is reported as cached tokens on Chat Completions, Responses and Messages and costed at the cache rates. Cache breakpoints are accepted and ignored, since the model rejects them. A turn replaying the model's own reasoning is served | ![Amazon Bedrock](styles/logo_amazon_bedrock.svg){: style="height:20px;width:20px"} Amazon Bedrock |
| ![Anthropic](styles/logo_anthropic.svg){: style="height:20px;width:20px"} **Anthropic** | [Claude Opus 5.5](api_anthropic_messages.md#extended-thinking) – a request disabling reasoning on Opus 5.5, which always reasons, is served at its adaptive default with a warning in the request log, as on Fable and Mythos, rather than refused. On Bedrock Mantle, `xhigh` and `max` reach Claude 4.7 and later unchanged instead of being capped to `high` | ![Amazon Bedrock](styles/logo_amazon_bedrock.svg){: style="height:20px;width:20px"} Amazon Bedrock |

#### :material-cash-minus: Cost Reporting
...
- **GPT-5.4 and GPT-5.5 are costed at their long-context rates past 272K input tokens**, the boundary AWS publishes for them, where every call was costed at the short rate. See [Long-Context Pricing](operations_cost_management.md#long-context-pricing).
...
- **Transcription moves on from a region that does not offer the operation.** Medical transcription and live streaming run in fewer regions than standard transcription; a region lacking one now hands the request to the next configured region instead of failing it, and when none offers it the answer is a `503`. See [Resilience](operations_resilience.md).
- **The [Models](models.md) page lists Claude Opus 5.5, GPT-6 Sol and GPT-6 Luna** (the last two without a price, as AWS publishes none yet), GPT-6 Astra with its prices, the GPT-5.4 and GPT-5.5 long-context tier, and the Global rates of Kimi K3 and Grok 4.6.

#### :material-bug: Fixes { #fixes-6 }
...
!!! abstract "At a glance"
    ...
    - **Tenant keys rotate themselves** — set [`TENANT_KEY_SECRETSMANAGER_PREFIX`](operations_configuration_authentication.md#tenant-key-secretsmanager-prefix) and each key becomes a Secrets Manager secret the tenant reads for itself, rotated on a schedule with an overlap window so nothing is locked out mid-flight.
    - **Per-tenant rate limits** — [`requests_per_minute` and `tokens_per_minute`](operations_authentication_security.md#tenant-rate-limits) per key or per deployment, counted across every instance. A deployment that declares none pays neither the latency nor the DynamoDB bill.
    - **Voice agents can act** — a [Realtime](api_openai_realtime.md) session declares tools and answers the calls the model makes, and [Chat Completions](api_openai_chat_completions.md) requests can ask for a server-side web search with OpenAI's own `web_search_options`.
...
!!! info "New IAM Permissions, All Optional"
    **Nothing new is required.** A deployment that enables neither feature below needs the policy it already has; the Terraform module grants each set when its feature is turned on. See [IAM Permissions](operations_iam_permissions.md).

    - **Tenant key rotation** — five Secrets Manager actions scoped to `<prefix>/*`, plus `kms:GenerateDataKey` and `kms:Decrypt` conditioned on the service and the encryption context, for [`TENANT_KEY_SECRETSMANAGER_PREFIX`](operations_configuration_authentication.md#tenant-key-secretsmanager-prefix). They replace the `ssm:*` delivery actions rather than adding to them. See [Tenant Key Rotation](operations_iam_permissions.md#tenant-key-rotation).
    - **Tenant rate limits** — `dynamodb:UpdateItem` on the shared table, in its own statement and conditioned on the counter items, so the one write-in-place action cannot touch a tenant record. Granted only when a limit is declared. See [Shared Table](operations_iam_permissions.md#shared-table).
...
| Provider                                                                        | Endpoint/Feature                                                                                                                      | AWS Backend                                                                                                                 |
|----------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------|
| ![OpenAI](styles/logo_openai.svg){: style="height:20px;width:20px"} **OpenAI** | [Realtime function calling](api_openai_realtime.md) – a session declares tools and answers the calls the model makes, so a voice agent can do more than talk | ![Amazon Bedrock](styles/logo_amazon_bedrock.svg){: style="height:20px;width:20px"} Amazon Bedrock - Amazon Nova Sonic |
| ![OpenAI](styles/logo_openai.svg){: style="height:20px;width:20px"} **OpenAI** | [`web_search_options` on Chat Completions](api_openai_chat_completions.md) – OpenAI's only spelling for server-side web search on the dialect most clients speak, resolved through the same declaration the Responses and Messages dialects already use. A model that runs no web search refuses rather than answering from its own knowledge, which nothing in the response would distinguish | ![Amazon Bedrock](styles/logo_amazon_bedrock.svg){: style="height:20px;width:20px"} Amazon Bedrock - Nova Grounding, Anthropic web search |
| ![OpenAI](styles/logo_openai.svg){: style="height:20px;width:20px"} **OpenAI** | [Prompt-attack and personal-data moderation](api_openai_moderations.md) – jailbreak and prompt-injection detection, and personal data detection, on `/v1/moderations` and the `moderation` parameter of the chat routes, with no guardrail resource to configure | ![Amazon Bedrock](styles/logo_amazon_bedrock.svg){: style="height:20px;width:20px"} Amazon Bedrock Guardrails |
| ![OpenAI](styles/logo_openai.svg){: style="height:20px;width:20px"} **OpenAI** | [`echo` on `/v1/completions`](api_openai_completions.md) – the prompt is echoed back ahead of the completion, which the gateway can do locally and was silently dropping | ![Amazon Bedrock](styles/logo_amazon_bedrock.svg){: style="height:20px;width:20px"} Amazon Bedrock - Converse API |
| ![Anthropic](styles/logo_anthropic.svg){: style="height:20px;width:20px"} **Anthropic** | [Opaque page tokens on `GET /v1/files`](api_anthropic_files.md) – `next_page` is served and `page` accepted, the way the SDK now pages. The token is opaque and refused when it is not one this API issued; the ID cursors stay | ![Amazon S3](styles/logo_amazon_s3.svg){: style="height:20px;width:20px"} Amazon S3 |
| ![Anthropic](styles/logo_anthropic.svg){: style="height:20px;width:20px"} **Anthropic** | [`expires_in_seconds` on a file upload](api_anthropic_files.md) – the full upstream range (1 hour to 90 days) is honoured and `expires_at` reported on all three routes. Expiry is enforced on every read, so a TTL longer than the storage sweep still expires at the instant promised | ![Amazon S3](styles/logo_amazon_s3.svg){: style="height:20px;width:20px"} Amazon S3 |
| ![Ollama](styles/logo_ollama.svg){: style="height:20px;width:20px"} **Ollama** | [`prompt_eval_cached_count`, and the base URL's connection test](api_ollama_chat.md) – the cached-prompt count Bedrock already reports is passed through, and the base URL answers `Ollama is running` rather than a `404`. `/api/blobs/{digest}` joins the store verbs this deployment refuses | ![Amazon Bedrock](styles/logo_amazon_bedrock.svg){: style="height:20px;width:20px"} Amazon Bedrock - Converse API |

#### :material-account-multiple: Multi-Tenancy

| Provider                                                                        | Endpoint/Feature                                                                                                                      | AWS Backend                                                                                                                 |
|----------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------|
| **stdapi.ai** | [Tenant key rotation](operations_authentication_security.md#tenant-api-keys) – with [`TENANT_KEY_SECRETSMANAGER_PREFIX`](operations_configuration_authentication.md#tenant-key-secretsmanager-prefix) each key is stored as a Secrets Manager secret the tenant reads for itself, so [`TENANT_KEY_ROTATION_DAYS`](operations_configuration_authentication.md#tenant-key-rotation-days) rotates it with nobody in the loop. The superseded key keeps working for [`TENANT_KEY_ROTATION_OVERLAP_SECONDS`](operations_configuration_authentication.md#tenant-key-rotation-overlap-seconds) — seven days by default, `0` for a hard cutover — and only the immediately previous key survives | ![AWS Secrets Manager](styles/logo_amazon_secrets_manager.svg){: style="height:20px;width:20px"} AWS Secrets Manager |
| **stdapi.ai** | [Per-tenant rate limits](operations_authentication_security.md#tenant-rate-limits) – `requests_per_minute` and `tokens_per_minute`, per key or as a deployment default, counted in the shared table so a limit holds across every instance. Tokens are admitted on an estimate and reconciled from the billed usage, which is what keeps a Realtime or MCP session from billing to a window that has ended. A counter that cannot be written fails closed with a `503`, never a `429` the client did not earn | ![Amazon DynamoDB](styles/logo_amazon_dynamodb.svg){: style="height:20px;width:20px"} Amazon DynamoDB |

#### :material-cash-minus: Cost Control

| Provider                                                                        | Endpoint/Feature                                                                                                                      | AWS Backend                                                                                                                 |
|----------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------|
| **stdapi.ai** | [`AWS_BEDROCK_GUARDRAIL_SCOPE_TURNS`](operations_configuration_bedrock.md#aws-bedrock-guardrail-scope-turns) – a guarded conversation is charged for everything the client replays, because Converse evaluates the whole history on every turn. This scopes input evaluation to the trailing user turns you name: a 6,401-character history bills 7 text units per policy and adds 409 ms, against 1 unit and 194 ms for the last turn alone. **Off by default and left off unless you decide otherwise** — scoping genuinely lowers detection, not just the bill: a prompt attack split across two turns is blocked today and is not blocked once only the last turn is tagged. Model output is always evaluated in full | ![Amazon Bedrock](styles/logo_amazon_bedrock.svg){: style="height:20px;width:20px"} Amazon Bedrock Guardrails |
...
!!! abstract "At a glance"
    ...
    - **A new dialect** — clients written for [the Ollama API](api_ollama_chat.md) reach Amazon Bedrock unchanged, chat, generate, embed and the discovery verbs.
    - **Per-tenant API keys** — one deployment [serves several tenants](operations_authentication_security.md), each scoped to the models and endpoints it may use, revocable on its own and free to [bring its own AWS account](operations_authentication_security.md#tenant-aws-credentials).
    - **Usage and spend from the API** — the [OpenAI Administration API](api_openai_organization_usage.md) reports what was used and what it cost, per model, per endpoint and per user. Off by default: it needs [`USAGE_API`](operations_configuration_observability.md#usage-api) and [`CLOUDWATCH_METRICS`](operations_configuration_observability.md#cloudwatch-metrics).
    - **WebRTC realtime** — [`POST /v1/realtime/calls`](api_openai_realtime.md#webrtc-calls) terminates a browser's media session in the gateway, with no WebSocket relay in between. Off by default: it needs [`REALTIME_WEBRTC_ENABLED`](operations_configuration_bedrock.md#realtime-webrtc-enabled).

This release widens what a deployment can serve, who it can serve it to, and what it can tell you about the bill. **Models you host yourself**: [Amazon SageMaker AI endpoints](operations_configuration_models.md#aws-sagemaker-endpoints) and [Amazon Bedrock Marketplace model endpoints](features.md#bedrock-marketplace-endpoints) you have already deployed join the model list and answer on the same routes as everything else — a fine-tuned model, a model no serverless catalogue carries, or one you scale to zero between requests, reached with the same client code. **A new dialect**: clients written for [the Ollama API](api_ollama_chat.md), and the many tools that speak only that, now reach Amazon Bedrock unchanged. **Per-tenant API keys**: one deployment [serves several tenants](operations_authentication_security.md), each scoped to the models and endpoints it may use, revocable on its own, and free to [bring its own AWS account](operations_authentication_security.md#tenant-aws-credentials). **Usage and spend, from the API**: the [OpenAI Administration API](api_openai_organization_usage.md) reports what was used and what it cost — per model, per endpoint and, where a deployment identifies callers, per user — read from the metrics the gateway itself writes rather than estimated. **Speaking to it, live**: the Realtime API gained the [WebRTC transport](api_openai_realtime.md#webrtc-calls), so a browser negotiates its own media session and the audio path terminates in the gateway, with no WebSocket relay in between.

!!! info "New IAM Permissions, All Optional"
    **Nothing new is required.** A deployment that enables none of the features below needs the policy it already has; the Terraform module grants each set when its feature is turned on. See [IAM Permissions](operations_iam_permissions.md) for the statements in full, and note that a denial now [names the action it needs](operations_troubleshooting.md) in the server log rather than leaving you to guess it.

    - **Your own SageMaker AI endpoints** — `sagemaker:CallWithBearerToken` and `sagemaker:InvokeEndpoint` on the endpoint ARNs you serve, for [Amazon SageMaker AI endpoints](operations_configuration_models.md#aws-sagemaker-endpoints).
    - **Your own Marketplace model endpoints** — `bedrock:ListMarketplaceModelEndpoints` and `bedrock:GetMarketplaceModelEndpoint` to discover [Marketplace model endpoints](features.md#bedrock-marketplace-endpoints), `sagemaker:InvokeEndpoint` and `sagemaker:InvokeEndpointWithResponseStream` (called via Bedrock on the server's behalf) to invoke them, and — for a deployment with per-user roles — `bedrock:InvokeModel` on `arn:aws:bedrock:*:ACCOUNT_ID:marketplace/model-endpoint/*`, which a `foundation-model` wildcard does not cover.
    - **Shared records** — the `dynamodb` item actions on one table, for [per-tenant API keys](operations_authentication_security.md) or the [shared model list](operations_resilience.md#model-list-refresh).
    - **Tenants** — `ssm:PutParameter` and `ssm:GetParameter` to deliver each tenant its key once, `kms:Encrypt`, `kms:Decrypt` and `kms:GenerateDataKey` on the key named by [`TENANT_KEY_SSM_KMS_KEY_ID`](operations_configuration_authentication.md#tenant-key-ssm-kms-key-id) when you set one, and `sts:AssumeRole` on exactly the roles tenants declare when they [bring their own AWS account](operations_authentication_security.md#tenant-aws-credentials).
    - **Usage reporting** — `cloudwatch:GetMetricData` and `cloudwatch:ListMetrics` for the [Administration API](api_openai_organization_usage.md).
    - **Per-user cost attribution roles** — one addition to an existing feature: the role the gateway assumes per end user now needs `s3:GetObject` (and `kms:Decrypt` through S3) on every bucket a request can reference by URI, because Amazon Bedrock reads a video or document at an `s3Location` with the invoking identity. Without it, a request that carries stored media answers `403` under that role while the same request under the task role succeeds. The module grants it; a hand-written role needs the statements in [Per-User Cost Attribution](operations_iam_permissions.md#per-user-cost-attribution).
...
!!! warning "Behavior Changes"
    ...
    - **Mantle routing and the per-user role's identity requirement are refused together.** A Mantle request is signed with the server's own credentials, so [`AWS_BEDROCK_USER_ROLE_REQUIRE_IDENTITY`](operations_configuration_bedrock.md#aws-bedrock-user-role-require-identity) can never apply to a model routed there: a deployment setting both with `AWS_BEDROCK_MANTLE_PREFERRED_MODELS` non-empty stops at startup, and a Mantle request without an end-user identity answers `400`.
    ...
    - **GPT-5.6 usage is reported and billed under Bedrock Mantle**, attributed by [project](operations_configuration_aws.md#bedrock-mantle-project) rather than by IAM principal, and runs on Mantle's own throughput quotas. Batch inference, prompt caching and response IDs stored before the upgrade are unaffected.
...
#### :material-server-network: Your Own Model Endpoints

| Provider                                                                        | Endpoint/Feature                                                                                                                      | AWS Backend                                                                                                                 |
|----------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------|
| **stdapi.ai** | [Amazon SageMaker AI endpoints](operations_configuration_models.md#aws-sagemaker-endpoints) – the endpoints you already run join the model list under the names you pick and answer on the same routes as every other model, streaming included: a fine-tuned model, or one no serverless catalogue carries, reached with unchanged client code | ![Amazon SageMaker AI](styles/logo_amazon_sagemaker.svg){: style="height:20px;width:20px"} Amazon SageMaker AI |
| **stdapi.ai** | [Endpoints scaled to zero](operations_configuration_models.md#aws-sagemaker-endpoints) – an endpoint with no running capacity is woken and waited for rather than refused; one with no capacity to wake answers `503` (retry later), not `400` | ![Amazon SageMaker AI](styles/logo_amazon_sagemaker.svg){: style="height:20px;width:20px"} Amazon SageMaker AI |
| **stdapi.ai** | [Amazon Bedrock Marketplace model endpoints](features.md#bedrock-marketplace-endpoints) – the endpoints you have deployed are discovered and served like any other model, invoked through Amazon Bedrock on the server's behalf, and one being updated stays served | ![Amazon Bedrock](styles/logo_amazon_bedrock.svg){: style="height:20px;width:20px"} Amazon Bedrock - Marketplace model endpoints |

#### :material-api: New APIs

| Provider                                                                        | Endpoint/Feature                                                                                                                      | AWS Backend                                                                                                                 |
|----------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------|
| ![Ollama](styles/logo_ollama.svg){: style="height:20px;width:20px"} **Ollama** | [`POST /ollama/api/chat`](api_ollama_chat.md) – conversational responses with tools, images and thinking, streamed as the newline-delimited JSON Ollama clients expect, carrying the timings they read | ![Amazon Bedrock](styles/logo_amazon_bedrock.svg){: style="height:20px;width:20px"} Amazon Bedrock - Converse API |
| ![Ollama](styles/logo_ollama.svg){: style="height:20px;width:20px"} **Ollama** | [`POST /ollama/api/generate`](api_ollama_generate.md) – a response for a single prompt, streamed or whole; an empty prompt, here or on `/api/chat`, is the load/unload no-op Ollama defines rather than a `400` | ![Amazon Bedrock](styles/logo_amazon_bedrock.svg){: style="height:20px;width:20px"} Amazon Bedrock - Converse API |
| ![Ollama](styles/logo_ollama.svg){: style="height:20px;width:20px"} **Ollama** | [`POST /ollama/api/embed` and `POST /ollama/api/embeddings`](api_ollama_embed.md) – vector embeddings for one or several inputs, including the legacy single-prompt route older clients still call | ![Amazon Bedrock](styles/logo_amazon_bedrock.svg){: style="height:20px;width:20px"} Amazon Bedrock - embedding models |
| ![Ollama](styles/logo_ollama.svg){: style="height:20px;width:20px"} **Ollama** | [`/ollama/api/tags`, `/ollama/api/show`, `/ollama/api/ps`, `/ollama/api/version` and `/ollama/api/pull`](api_ollama_models.md) – the discovery verbs an Ollama tool calls before anything else. The verbs that write to a local model store — `create`, `copy`, `push`, `delete` — are refused, since this deployment stores no models of its own. The `/api/blobs/{digest}` upload joined that refused set in v1.18.0, which also answers `Ollama is running` on the base URL for a client's connection test | ![Amazon Bedrock](styles/logo_amazon_bedrock.svg){: style="height:20px;width:20px"} Amazon Bedrock |
| ![OpenAI](styles/logo_openai.svg){: style="height:20px;width:20px"} **OpenAI** | [`GET /v1/organization/usage/*`](api_openai_organization_usage.md) – tokens, images, characters and seconds per bucket, per model, per endpoint and — where a deployment identifies callers — per user, read from the metrics the gateway itself writes. An endpoint that measures nothing answers empty buckets rather than a `404`. Off by default: it needs [`USAGE_API`](operations_configuration_observability.md#usage-api) and [`CLOUDWATCH_METRICS`](operations_configuration_observability.md#cloudwatch-metrics), and grouping by user needs [`CLOUDWATCH_METRICS_USER_DIMENSION`](operations_configuration_observability.md#cloudwatch-metrics-user-dimension) as well | ![Amazon CloudWatch](styles/logo_amazon_cloudwatch.svg){: style="height:20px;width:20px"} Amazon CloudWatch |
| ![OpenAI](styles/logo_openai.svg){: style="height:20px;width:20px"} **OpenAI** | [`GET /v1/organization/costs`](api_openai_organization_usage.md) – what AWS bills this deployment for serving those requests, over the same buckets and the same filters. Needs [`COST_TRACKING`](operations_configuration_observability.md#cost-tracking) on top of the two settings above | ![Amazon CloudWatch](styles/logo_amazon_cloudwatch.svg){: style="height:20px;width:20px"} Amazon CloudWatch |
| ![OpenAI](styles/logo_openai.svg){: style="height:20px;width:20px"} **OpenAI** | [`POST /v1/realtime/calls`](api_openai_realtime.md#webrtc-calls) – the WebRTC transport: a browser posts its own SDP offer, negotiates a media session directly with the gateway and the audio path terminates there, with no WebSocket relay in between. An offer whose ICE candidates are all private, link-local or mDNS names is refused with `invalid_offer`; a deployment on a private network sets [`REALTIME_WEBRTC_ALLOW_PRIVATE_CANDIDATES`](operations_configuration_bedrock.md#realtime-webrtc-allow-private-candidates). Off by default: it needs [`REALTIME_WEBRTC_ENABLED`](operations_configuration_bedrock.md#realtime-webrtc-enabled), and the task must be directly reachable over UDP — behind an HTTP-only load balancer the SDP exchange succeeds but the call carries no audio | ![Amazon Bedrock](styles/logo_amazon_bedrock.svg){: style="height:20px;width:20px"} Amazon Bedrock - Amazon Nova Sonic |

#### :material-account-multiple: Multi-Tenancy

| Provider                                                                        | Endpoint/Feature                                                                                                                      | AWS Backend                                                                                                                 |
|----------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------|
| **stdapi.ai** | [Per-tenant API keys](operations_authentication_security.md#tenant-api-keys) – issue a key per tenant, scope it to the models and endpoints that tenant may use, and revoke it without touching anyone else. Files, vector stores, batches, conversations and stored responses stay deployment-wide with no tenant ownership — a [documented limitation](operations_authentication_security.md#tenant-api-keys) of this release | ![Amazon DynamoDB](styles/logo_amazon_dynamodb.svg){: style="height:20px;width:20px"} Amazon DynamoDB |
| **stdapi.ai** | [Key delivery that skips Terraform state](operations_authentication_security.md#tenant-api-keys) – the server mints each tenant's secret and delivers it once through a parameter, encrypted with the key named by [`TENANT_KEY_SSM_KMS_KEY_ID`](operations_configuration_authentication.md#tenant-key-ssm-kms-key-id) — the deployment's own KMS key when the Terraform module provisions it — so reading it takes a grant on that key and not merely permission on the parameter path | ![AWS Systems Manager](styles/logo_amazon_systems_manager.svg){: style="height:20px;width:20px"} AWS Systems Manager Parameter Store, AWS KMS |
| **stdapi.ai** | [Tenant AWS credentials](operations_authentication_security.md#tenant-aws-credentials) – a tenant may bring its own AWS account: its requests then run under its own role, its own Amazon Bedrock quota and its own bill | ![Amazon Bedrock](styles/logo_amazon_bedrock.svg){: style="height:20px;width:20px"} Amazon Bedrock, AWS STS |

#### :material-magnify: Model Discovery

| Provider                                                                        | Endpoint/Feature                                                                                                                      | AWS Backend                                                                                                                 |
|----------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------|
| **stdapi.ai** | [Wildcard model names](operations_configuration_models.md#model-wildcard-patterns) – any request that names a model may name a pattern instead — `claude-sonnet-*`, say — and the most recently released match serves it; an ambiguous tie is refused rather than guessed | ![Amazon Bedrock](styles/logo_amazon_bedrock.svg){: style="height:20px;width:20px"} Amazon Bedrock |
| **stdapi.ai** | [`GET /search_models`](api_search_models.md) – a `model=` filter lists everything a pattern matches, newest first, so a pattern can be checked before anything relies on it | ![Amazon Bedrock](styles/logo_amazon_bedrock.svg){: style="height:20px;width:20px"} Amazon Bedrock |
| **stdapi.ai** | [Shared model list](operations_resilience.md#model-list-refresh) – a fleet discovers models once and shares the result, instead of once per task, and the refresh stays off the request path | ![Amazon DynamoDB](styles/logo_amazon_dynamodb.svg){: style="height:20px;width:20px"} Amazon DynamoDB |

#### :material-tune: API Improvements

| Provider                                                                        | Endpoint/Feature                                                                                                                      | AWS Backend                                                                                                                 |
|----------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------|
| ![OpenAI](styles/logo_openai.svg){: style="height:20px;width:20px"} **OpenAI** | [Diarized transcripts streamed](api_openai_audio_transcriptions.md#streaming) – a diarized transcription is returned as speaker-labelled segments as they are recognized, instead of being refused | ![Amazon Transcribe](styles/logo_amazon_transcribe.svg){: style="height:20px;width:20px"} Amazon Transcribe |
| ![Cohere](styles/logo_cohere.svg){: style="height:20px;width:20px"} **Cohere** | [Fused multimodal embeddings](api_cohere_embed.md) – text and an image embedded together into one vector, on both embed routes | ![Amazon Bedrock](styles/logo_amazon_bedrock.svg){: style="height:20px;width:20px"} Amazon Bedrock - embedding models |
| **stdapi.ai** | [Cohere Embed v3 listed as image-capable](api_search_models.md) – Bedrock publishes `cohere.embed-english-v3` and `cohere.embed-multilingual-v3` as text-only although both embed an image and are billed for it; the catalogue now declares the modalities the listing omits, so `/v1/models`, `/search_models` and the Models page agree with what is served | ![Amazon Bedrock](styles/logo_amazon_bedrock.svg){: style="height:20px;width:20px"} Amazon Bedrock - embedding models |

#### Platform Features

| Feature                                | Description                                                                                                                                                                             |
|----------------------------------------|-------------------------------------------------------------------------------------------------------------------------------------|
| [Public Models page](https://stdapi.ai/models/) | Browse, filter and compare every model the gateway serves, with each fact resolved from the source that states it |
| [Denials name the permission they need](operations_troubleshooting.md) | An AWS authorization failure is logged with the action that was denied, instead of leaving the operator to infer it from the call that failed |
...
| WebRTC media path provisioned | Off by default: it needs [`REALTIME_WEBRTC_ENABLED`](operations_configuration_bedrock.md#realtime-webrtc-enabled) on the server and [`realtime_webrtc_media_enabled`](operations_deploy_advanced.md#webrtc-and-sip-need-their-own-ingress) on the Terraform module. In media mode the module provisions the whole UDP path on the VPC it creates — the matching egress, and the application-subnet network ACL entries the media needs; bring your own `subnet_ids` and those entries stay yours to widen. The module's `realtime_webrtc_stun_server` defaults to a Google-operated public STUN server, which the task reaches over an egress rule open to `0.0.0.0/0` on that port — the one place this deployment talks to a third party. Point it at a STUN server you run to keep the flow inside your account |
...
| [Per-user attribution role, built by the module](operations_cost_management.md#per-user-attribution) | `aws_bedrock_user_role_create` creates the role the gateway assumes per end user — its trust policy and the invoke permissions AWS authorizes against the caller included — instead of leaving it to be written by hand and named by ARN; `aws_bedrock_user_role_arn` still takes an existing one and wins where both are set. Off by default, as the feature itself is |
| Scales on CPU out of the box | A deployment that sets no autoscaling variable now scales on CPU at 70%, between one task per Availability Zone and five times that — [size the ceiling](operations_cost_management.md) before it surprises the bill |
| [Home Assistant voice, end to end](use_cases_home_assistant.md) | The deployment sample now configures the conversation agent as well as speech, so a spoken command is understood and acted on rather than only transcribed and spoken back |

#### :material-bug: Fixes { #fixes-4 }
...
- **A model both catalogues name stays reachable through bedrock-runtime**: naming a dual-homed model in [`AWS_BEDROCK_MANTLE_PREFERRED_MODELS`](operations_configuration_models.md#bedrock-mantle-preferred-models) published the preferred entry under that identifier and dropped the other, so the model was reported impossible to batch — batch inference runs on bedrock-runtime alone. The displaced bedrock-runtime entry is kept off-catalogue and read by everything that must reach that endpoint, while the published catalogue keeps naming the preferred entry. It now matters to every deployment, that setting having gained a default.
