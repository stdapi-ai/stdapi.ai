...
- **Errors never name the deployment's account.** A revoked trust policy, a wrong ExternalId or an expired session answers a fixed `403` — *"The AWS credential registered for this API key could not be used…"* — with the full AWS detail written to the server log only.
