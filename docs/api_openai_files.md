...
- :material-format-list-bulleted: **Pages of up to 10 000 files** — ascending or
  descending, paged with the `after` cursor and filterable by `purpose`, see
  [List files](#list-files).
- :material-file-document-multiple: **Referenced by `file_id` in Chat
  Completions** — a `"type": "file"` content part carries the uploaded file into
  the conversation, see
  [Chat completions integration](#chat-completions-integration).
- :material-plus-circle: **JSON uploads beyond the OpenAI API** — a base64
  string, data URI, HTTPS URL or S3 URI stands in for the binary form field for
  clients that cannot build `multipart/form-data`, see
  [Upload via JSON body](#upload-via-json-body-mcp-and-ai-agents).
- :material-link-variant: **A project-local `file-id:` URI scheme** — it
  references an uploaded file on string fields that have no typed `file_id`
  slot, see
  [Referencing uploaded files](#referencing-uploaded-files-via-the-file-id-uri-scheme).
...
Files are stored in S3 under the prefix configured by [`AWS_S3_FILES_PREFIX`](operations_configuration_storage.md#aws-s3-files-prefix) (default: `files/`). Configure S3 Lifecycle rules on this prefix to automatically delete expired objects and apply Intelligent-Tiering.

## Limits and behaviour to know

Size and part caps sit with the capability they qualify: the direct-upload
ceiling in [Feature compatibility](#feature-compatibility), the session size,
part size and part count rules under [Uploads API](#uploads-api), and the
...
Next: [Anthropic Files API](api_anthropic_files.md) · [Chat Completions API](api_openai_chat_completions.md) · [Batch API](api_openai_batches.md) · [Storage configuration](operations_configuration_storage.md#aws-s3-files-prefix)
