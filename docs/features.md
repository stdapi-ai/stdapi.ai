---
title: Features - OpenAI, Anthropic, Cohere & Ollama Compatible AI Gateway for Amazon Bedrock
description: stdapi.ai features — OpenAI, Anthropic, Cohere and Ollama API compatibility for Amazon Bedrock & AI services. Multi-region access, compliance controls, security, observability, and more.
keywords: AI gateway features, AWS Bedrock gateway, OpenAI API compatible, Anthropic API compatible, Cohere API compatible, Ollama API compatible, OpenAI to Bedrock, Anthropic to Bedrock, Cohere to Bedrock, Ollama to Bedrock, AWS AI services, private AI gateway, prompt caching, guardrails, data sovereignty, enterprise AI, multi-region AI, S3 integration, OpenTelemetry AI, OpenAI alternative AWS, Anthropic alternative AWS, Bedrock Mantle models, OpenAI GPT on AWS, xAI Grok on AWS
---
...
- :material-api: __[API Compatibility](#api-compatibility)__
  <br>100+ endpoints across four protocols, each one backed by an AWS service.

- :material-brain: __[Models](#100-models-across-20-providers)__
  <br>Every model on Amazon Bedrock through one consistent API, plus the Mantle-only frontier models.

- :material-image-multiple: __[Multi-Modal Capabilities](#multi-modal-capabilities)__
  <br>Text, images, audio, video, documents, embeddings and retrieval.

- :material-aws: __[Purpose-Built for AWS](#purpose-built-for-aws)__
  <br>Multi-region routing, Bedrock features, Mantle, Marketplace and SageMaker AI endpoints, Amazon S3.

- :material-shield-lock: __[Security & Compliance](#security-compliance)__
  <br>Authentication, security controls, compliance and data sovereignty.

- :material-puzzle: __[Works with Your Existing Tools](#works-with-your-existing-tools)__
  <br>The clients, SDKs and platforms that already speak these protocols.

- :material-robot: __[AI Agents](#ai-agents)__
  <br>Agent discovery and the Model Context Protocol.

- :material-chart-line: __[Observability & Operations](#observability-operations)__
  <br>Logging, tracing, cost tracking and per-user cost attribution.

- :material-speedometer: __[Performance](#performance)__
  <br>Measured gateway CPU over the complete request path.

- :material-clipboard-check: __[Quality Assurance](#quality-assurance)__
  <br>How the compatibility claims on this page are tested.

- :material-rocket-launch: __[Deployment](#deployment)__
  <br>The Community and Commercial editions, side by side.

- :material-scale-balance: __[How stdapi.ai Compares](#how-stdapiai-compares)__
  <br>Feature-by-feature against the alternatives.
...
Anthropic, Cohere and Ollama routes live under `/anthropic`, `/cohere` and `/ollama`, so the four protocols are served side by side without colliding on `/v1` — and every prefix can become the path your clients already send ([Anthropic](operations_configuration_server.md#anthropic-routes-prefix), [Cohere](operations_configuration_server.md#cohere-routes-prefix), [Ollama](operations_configuration_server.md#ollama-routes-prefix)).
...
- ![Cohere](styles/logo_cohere.svg){ style="height: 1.2em; vertical-align: text-bottom;" } **Cohere**
  <br>Embed v3 and v4 for multimodal embeddings; Rerank 3.5 for reranking.
...
- **Aliases** — A model is published under whichever names you choose, with Claude and OpenAI names resolving on their own; an alias can carry [its own service tier, guardrail, metadata and parameters](operations_configuration_models.md#model-aliases-configuration), so one model serves several policies under several names
- **Wildcard model names** — Any request that names a model can name a [glob pattern](operations_configuration_models.md#model-wildcard-patterns) instead (`claude-sonnet-*`), and the newest release that matches serves the request — resolved once, at creation, so a pinned job keeps its model for its whole life
- **Deprecation handled for you** — Models AWS has retired drop out of the list your users pick from, so nobody builds on a model about to be withdrawn, and requests naming one are redirected to its replacement; a workload that still depends on one can [keep it listed](operations_configuration_models.md#bedrock-legacy)
...
- [Function calling](api_openai_realtime.md#function-calling): the model asks for one of the session's own functions, the application runs it, and the answer is spoken with the result
...
| Above it, on a model that reads that kind from storage      | Staged in [a bucket of yours](operations_configuration_storage.md#aws-s3-regional-buckets) in the region serving the request, and referenced |
...
- **Held in your own account** — Documents, passages and vectors live in an [Amazon S3 vector bucket](operations_configuration_storage.md#aws-s3-vectors-bucket) in your account, not in an index somebody else runs
...
A deployment spanning several AWS regions draws on more than one Bedrock quota and keeps serving when one region is degraded. How traffic spreads across them is yours to [choose](operations_configuration_aws.md#bedrock-region-routing), and the choice trades throughput against prompt-cache hit rate:
...
| **Web Search / Grounding**         | Built-in web search with source citations, billed per query: Amazon Nova grounding (Chat Completions, Responses, and Messages) and OpenAI GPT built-in search (`/v1/responses` only). A deployment can [keep grounding off the open internet](operations_configuration_models.md#bedrock-external-web-access) entirely, and a request asking for what it forbids is refused rather than silently rewritten |
...
- **Routing you choose** — A model available on both endpoints is served by the classic one, except the **OpenAI GPT-5.6 and GPT-6 families**, which are served by Mantle where it lists them so that their [built-in web search](api_openai_responses.md#openai-gpt-web-search) and code interpreter work without configuration; Mantle serves the models only it has. Any dual-homed model can be [pointed at Mantle](operations_configuration_models.md#bedrock-mantle-preferred-models) — or taken back off it — for the whole deployment, or routed for a single request, tapping Mantle's separate throughput quotas on top of your Bedrock ones
...
!!! warning "Price change for the OpenAI GPT-5.6 and GPT-6 families"
    Serving GPT-5.6 Sol, Terra and Luna and GPT-6 Astra on Mantle is **a price change**, not only a routing one. Both endpoints charge the same In-Region rate, but Mantle has no cross-region inference profiles, so these models no longer ride the Global profile and its discount: **every token costs exactly 10% more** than on the classic endpoint's default Global routing — input, output, cached and long-context rates alike. The per-million figures are in the [`AWS_BEDROCK_MANTLE_PREFERRED_MODELS`](operations_configuration_models.md#bedrock-mantle-preferred-models) reference. A deployment that pins In-Region routing pays what it paid. GPT-6 Sol and Luna have no published rate yet, so their usage is recorded without a cost.

    Three other things change with them: Amazon Bedrock **Guardrails cannot apply** (a deployment configuring both is [refused at startup](operations_configuration_models.md#bedrock-mantle-preferred-models)), and usage and cost are reported — and billed by AWS — under Bedrock Mantle rather than Bedrock, attributed by [project](operations_configuration_aws.md#bedrock-mantle-project). Throughput runs on Mantle's own quotas. Batch inference, prompt caching and stored responses are unaffected: batches still run on the classic endpoint, both endpoints cache, and response IDs issued before the change keep working.

    Set [`AWS_BEDROCK_MANTLE_PREFERRED_MODELS`](operations_configuration_models.md#bedrock-mantle-preferred-models) to an empty value to serve every dual-homed model on the classic endpoint, at the classic price and under your guardrail.
...
[:octicons-arrow-right-24: Bedrock Mantle Configuration](operations_configuration_aws.md#bedrock-mantle-enabled)
...
- **Opt-in and region-scoped** — Off unless you [enable it](operations_configuration_models.md#bedrock-marketplace-endpoints-enabled), and only in the regions you serve: an endpoint is invoked in its own region, so one deployed elsewhere is never published
...
[:octicons-arrow-right-24: Marketplace Model Endpoints Configuration](operations_configuration_models.md#bedrock-marketplace-endpoints-enabled)
...
- **Named, never deployed** — You [name the endpoints](operations_configuration_models.md#aws-sagemaker-endpoints) you want served, with the model ID your clients should ask for. The gateway invokes them and nothing else: it never creates, updates, scales or deletes an endpoint, because that is capacity you pay for and infrastructure you own
...
- **Per-caller identity** — [Amazon Cognito user pool tokens](operations_configuration_authentication.md#cognito-authentication) are accepted instead of, or alongside, the API key, so each caller reaches the API with their own credential — validated in-process against the pool's published keys, with no AWS call on the request path, and it is that verified identity [per-user cost attribution](#per-user-cost-attribution) bills against
- **The posture is asserted, not inferred** — [Name the method you intend to run](operations_configuration_authentication.md#authentication-mode): the server refuses to start when the method you named is not actually in force, or when one you configured would be silently ignored, so a deployment cannot drift into answering unauthenticated traffic
- **Agents authenticate without being configured** — Every unauthorized response points at the [document](operations_configuration_authentication.md#oauth-discovery) naming the authorization server and the scope this deployment expects
...
- **Malformed requests stop at the edge** — Out-of-spec requests are rejected before they reach an AWS call, under a [strict mode](operations_configuration_observability.md#strict-input-validation) that also refuses unknown fields instead of ignoring them
...
[:octicons-arrow-right-24: MCP Configuration](operations_configuration_server.md#mcp-model-context-protocol)
...
- **Prompts stay out of your logs unless you ask for them** — Full request and response payloads and the client IP are [available when you are debugging](operations_configuration_observability.md#log-request-params) and are not written otherwise
...
- **Usage and cost reporting in the shape your tools already read** — Consumption and spend come back from the [OpenAI Administration usage and costs endpoints](api_openai_organization_usage.md), answered from the metrics the deployment publishes to Amazon CloudWatch. It reports the whole deployment, so it is an administrator surface: [off by default](operations_configuration_observability.md#usage-api), and never readable with a tenant key
...
Every compatibility claim on this page is tested against the real vendor APIs and driven by real client software — [see the evidence](compare.md#quality-assurance).
...
| **Container image**           | <span class="m-p" aria-hidden="true">◐</span> Standard Debian-slim build (GHCR) | <span class="m-y" aria-hidden="true">✓</span> Hardened minimal base, AWS Marketplace security-validated      |
...
| **Security posture**          | Manual (self-managed)                                   | <span class="m-y" aria-hidden="true">✓</span> Built against Security Hub FSBP, passing most applicable controls out of the box; GuardDuty & DNS Firewall integrations |
...
Feature by feature against the other ways to put an OpenAI-compatible API in front of Amazon Bedrock, with sources and verification dates — [see the comparison](compare.md#how-stdapiai-compares).

---

Next: [Get Started](getting_started.md) · [Deploy on AWS](operations_getting_started.md) · [Run Locally](operations_getting_started_local.md) · [API Reference](api_overview.md) · [Models](models.md)
