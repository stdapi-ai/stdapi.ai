...
- **Wildcard model names** — Any request that names a model can name a [glob pattern](operations_configuration.md#model-wildcard-patterns) instead (`claude-sonnet-*`), and the newest release that matches serves the request — resolved once, at creation, so a pinned job keeps its model for its whole life
...
!!! warning "Price change for the OpenAI GPT-5.6 family"
    Serving GPT-5.6 Sol, Terra and Luna on Mantle is **a price change**, not only a routing one. Both endpoints charge the same In-Region rate, but Mantle has no cross-region inference profiles, so these models no longer ride the Global profile and its discount: **every token costs exactly 10% more** than on the classic endpoint's default Global routing — $4.40 / $22.00 per million input / output tokens for Sol, $2.20 / $13.20 for Terra, $0.22 / $1.32 for Luna, against $4.00 / $20.00, $2.00 / $12.00 and $0.20 / $1.20 before. The same 10% applies to cached tokens and to the long-context rates. A deployment that pins In-Region routing pays what it paid.
