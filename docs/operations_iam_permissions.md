...
Required, together with the [shared table permissions](#shared-table), when [tenant API keys](operations_authentication_security.md#tenant-api-keys) are enabled and keys are delivered through Parameter Store — the default. A deployment that sets [`TENANT_KEY_SECRETSMANAGER_PREFIX`](operations_configuration_authentication.md#tenant-key-secretsmanager-prefix) needs the [rotation statements](#tenant-key-rotation) instead, and none of these. The gateway writes each minted key exactly once (`PutParameter` refuses to overwrite), and reads a parameter back only to recover a mint that crashed between delivery and recording. Grant it on the delivery prefix and nothing wider.
