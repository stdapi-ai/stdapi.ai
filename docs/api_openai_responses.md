...
| `additional_tools` items                                              |   :material-check-circle:{ .success role="img" aria-label="Supported" }   | Input-history tolerance only: accepted and dropped on replay — the tools a turn may call are the ones declared in its own `tools`. Stored in a `conversation` and listed back like any other item |
| **Tool Calling**                                                      |                                         |                                                                              |
...
| `tool_choice: "required"`                                             |      :material-cog:{ .model-dep role="img" aria-label="Model-dependent" }       | Model must call at least one tool; Claude Opus 5.5 and later refuse it with `400` |
| `tool_choice: "none"`                                                 |   :material-minus-circle:{ .partial role="img" aria-label="Partial" }   | Withdraws every declared tool; an input already containing a `function_call` item keeps the tool it names callable, and the model may call it again |
| Named `tool_choice` (force)                                           |      :material-cog:{ .model-dep role="img" aria-label="Model-dependent" }       | Force a specific function to be called; Claude Opus 5.5 and later refuse it with `400` |
...
| Changing `tools` between turns                                        |   :material-check-circle:{ .success role="img" aria-label="Supported" }   | The tools declared on a turn are the only ones the model may call, whatever the replayed input names |
...
| `reasoning` (effort)                                                  |      :material-cog:{ .model-dep role="img" aria-label="Model-dependent" }       | Configures reasoning; without `effort` defaults to `medium`; `effort: "none"` disables, except on the models that always reason — levels and per-model handling as for Chat Completions [`reasoning_effort`](api_openai_chat_completions.md#reasoning-control); chain of thought returned as `reasoning` output items |
| `reasoning.summary` / `generate_summary`                              |      :material-cog:{ .model-dep role="img" aria-label="Model-dependent" }       | Any value returns the reasoning in the item's `summary` as `summary_text` parts instead of `content`; Anthropic Claude also returns its summarized reasoning, which recent Claude models otherwise omit. Forwarded upstream on Bedrock Mantle native models |
...
| `conversation`                                                        |   :material-check-circle:{ .success role="img" aria-label="Supported" }   | Prepends the conversation's items to `input` and appends the turn to it unless `store` is false; an input item no conversation item type can express (`compaction_trigger`, `item_reference`) is left out of the conversation; rejected (`400`) with `previous_response_id` — see [Conversations](api_openai_conversations.md) |
...
| `response.reasoning_summary_part.added` / `.done`, `response.reasoning_summary_text.delta` / `.done` |      :material-cog:{ .model-dep role="img" aria-label="Model-dependent" }       | Reasoning deltas on reasoning models when `reasoning.summary` is set         |
...
The chain of thought is returned as a `reasoning` output item preceding the assistant message. Where the text goes depends on the request:

| Request                                                   | Reasoning item                                                     | Streamed through                                                                                                                                  |
|-----------------------------------------------------------|--------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------|
| `reasoning.summary` set (`auto`, `concise` or `detailed`) | `summary` holds a `summary_text` part; `content` is empty          | `response.reasoning_summary_part.added`, `response.reasoning_summary_text.delta` / `.done`, `response.reasoning_summary_part.done`                |
| No summary requested                                      | `content` holds a `reasoning_text` part; `summary` is empty        | `response.content_part.added`, `response.reasoning_text.delta` / `.done`, `response.content_part.done`                                            |

Either way, the events sit between the item's `response.output_item.added` and `.done`, before the message events. Clients that only display `summary`, such as Codex by default, need `reasoning.summary` set to see the reasoning. On models that do not summarize their reasoning, the summary is the reasoning text they return. Token accounting depends on the serving path:

- **Converse-served models**: `usage.output_tokens_details.reasoning_tokens` is always `0`; reasoning tokens are billed inside `output_tokens`.
- **Mantle-native models**: without a summary request, the reasoning is an encrypted-content item bracketed only by `response.output_item.added` / `.done`. `usage.output_tokens_details.reasoning_tokens` is the count the model reports.
- **Mantle models converted to another API**, Anthropic Claude included: the reasoning comes back as described above, with the token accounting of Converse-served models.

Claude Opus 4.7 and later, Sonnet 5, Fable and Mythos reason without returning the text unless the request sets `reasoning.summary`: their summarized reasoning then comes back in `summary`. The reasoning tokens are billed the same with or without it. The Chat Completions API has no summary request, so these models return no reasoning text there.
