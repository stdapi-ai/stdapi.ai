...
class TestDerivedMetadata:
    ...
    def test_capabilities_are_derived_from_the_routes_and_modalities(self) -> None:
        ...
        model = make_model_details(
            ...
            supported_routes=[ollama_route("/api/chat")],
        )
    ...
    def test_thinking_and_insert_are_never_advertised(self) -> None:
        ...
        """
        model = make_model_details(
            "vendor.chat-v1", supported_routes=[ollama_route("/api/chat")]
        )
        capabilities = model_capabilities(model)
        assert capabilities, "expected a model that really advertises capabilities"
        assert "thinking" not in capabilities
        assert "insert" not in capabilities
    ...
    def test_an_embedding_model_advertises_embedding_alone(self) -> None:
        ...
        model = make_model_details(
            ...
            supported_routes=[ollama_route("/api/embed")],
        )
