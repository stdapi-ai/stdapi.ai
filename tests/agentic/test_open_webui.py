...
def _environment(server: AgenticServer, port: int) -> Mapping[str, str]:
    ...
    return {
        ...
        # -- Ollama Connection ------------------------------------------------
        # The base URL carries the gateway's Ollama prefix and nothing more: Open
        # WebUI appends /api/version, /api/tags and the rest itself. The key travels
        # inside the per-connection configuration rather than in a variable of its
        # own, because a local Ollama needs no credentials and the variable
        # therefore does not exist.
        "ENABLE_OLLAMA_API": "true",
        "OLLAMA_BASE_URL": f"{base_url}{SETTINGS.ollama_routes_prefix}",
