...
async def secrets_backend(
    ...
    async with session.create_client(
        ...
        monkeypatch.setattr(SETTINGS, "tenant_key_rotation_overlap_seconds", 604800.0)
...
class TestRotationConfiguration:
    ...
    def test_the_store_alone_is_a_complete_configuration(self) -> None:
        ...
        assert settings.tenant_key_rotation_overlap_seconds == 604800.0
    ...
    def test_a_negative_overlap_fails_startup(self) -> None:
        ...
        with pytest.raises(
            ...
            _settings(tenant_key_rotation_overlap_seconds=-1.0)
...
class TestOverlap:
    ...
    async def test_both_keys_authenticate_during_the_overlap(
        ...
        assert until == rotated + int(SETTINGS.tenant_key_rotation_overlap_seconds), (
            "the window is the configured one, not merely non-zero"
        )
    ...
    async def test_a_zero_overlap_is_a_hard_cutover(
        ...
        Ref: stdapi/tenant_keys.py:_rotate
        """
        monkeypatch.setattr(SETTINGS, "tenant_key_rotation_overlap_seconds", 0.0)
        key_id = "o" + "3" * 15
        old, new = await _rotate_once(secrets_backend, key_id)
        tenant_keys._CACHE.clear()  # noqa: SLF001
