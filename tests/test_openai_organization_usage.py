...
class TestStartupWarnings:
    ...
    def test_metrics_off_refuses_to_start_rather_than_warning(self) -> None:
        """The usage endpoints read what ``cloudwatch_metrics`` publishes.

        A deployment without them answers every query with empty buckets, which
        is worth refusing rather than reporting, so no warning covers this and
        the server never reaches the point of emitting one.

        Ref: stdapi/config.py:_validate_usage_api
        """
        from pydantic import ValidationError  # noqa: PLC0415

        from stdapi.config import _Settings  # noqa: PLC0415

        with pytest.raises(ValidationError, match="usage_api requires"):
            _Settings(usage_api=True, cloudwatch_metrics=False)
