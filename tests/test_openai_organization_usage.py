...
class TestMeasurements:
    ...
    def test_web_searches_are_counted_from_the_search_metrics(
        self, app_client: TestClient, cloudwatch: _FakeCloudWatch, recent_start: int
    ) -> None:
        """Requests on a model with no search metric are not counted as searches."""
        cloudwatch.results = [
            _series("SearchUnits", "search.model|responses", {recent_start: 3.0}, "q0"),
            _series("Requests", "search.model|responses", {recent_start: 3.0}, "q2"),
            _series("Requests", "chat.model|responses", {recent_start: 9.0}, "q2"),
        ]
        rows = _get(
    ...
    def test_num_model_requests_counts_the_whole_model_when_traffic_is_mixed(
        ...
        cloudwatch.results = [
            _series(
                "GroundingRequests", "chat.model|responses", {recent_start: 10.0}, "q1"
            ),
            _series("Requests", "chat.model|responses", {recent_start: 1000.0}, "q2"),
        ]
        rows = _get(
            app_client, "web_search_calls", start_time=recent_start, limit=1
        ).json()["data"][0]["results"]
