...
class TestAnthropicMessages:
    ...
    def test_code_execution_tool_rejected(
        self, anthropic_client: Anthropic, anthropic_chat_model: str
    ) -> None:
        """Test that code_execution tool is rejected on Bedrock Claude models.

        Validates:
            - code_execution is not supported via Bedrock and raises BadRequestError
        """
        with pytest.raises(BadRequestError):
            anthropic_client.messages.create(
                model=anthropic_chat_model,
    ...
    @pytest.mark.expensive
    def test_bash_tool_accepted(
        self,
        anthropic_client: Anthropic,
        anthropic_chat_model: str,
        use_anthropic_api: bool,
    ) -> None:
        """Test bash tool is accepted on Claude 3.7+ models (versioned name bash_20250124).

        Validates:
            - Bash tool definition is accepted
            - Request does not raise an error
        """
        response = anthropic_client.messages.create(
            model=anthropic_chat_model,
            max_tokens=4096,
    ...
    @pytest.mark.expensive
    def test_memory_tool_accepted(
        self,
        anthropic_client: Anthropic,
        anthropic_chat_model: str,
        use_anthropic_api: bool,
    ) -> None:
        """Test memory tool is accepted on Claude 3.7-4.5 models (versioned name memory_20250818).

        Validates:
            - memory tool definition is accepted (requires context-management-2025-06-27 beta, auto-injected)
            - Request does not raise an error
        """
        extra_headers = (
    ...
    def test_web_search_tool_rejected(
        self, anthropic_client: Anthropic, anthropic_chat_model: str
    ) -> None:
        """Test that web_search tool is rejected on Bedrock Claude models.

        Validates:
            - web_search is not supported via Bedrock and raises BadRequestError
        """
        with pytest.raises(BadRequestError):
            anthropic_client.messages.create(
                model=anthropic_chat_model,
    ...
    @pytest.mark.expensive
    def test_str_replace_based_edit_tool_tool_accepted(
        self,
        anthropic_client: Anthropic,
        anthropic_chat_model: str,
        use_anthropic_api: bool,
    ) -> None:
        """Test str_replace_based_edit_tool (text editor) tool is accepted on Claude models.

        Validates:
            - Text editor tool definition is accepted (versioned name text_editor_20250728)
            - Request does not raise an error
        """
        response = anthropic_client.messages.create(
            model=anthropic_chat_model,
    ...
    def test_web_fetch_tool_rejected(
        self, anthropic_client: Anthropic, anthropic_chat_model: str
    ) -> None:
        """Test that web_fetch tool is rejected on Bedrock Claude models.

        Validates:
            - web_fetch is not supported via Bedrock and raises BadRequestError
        """
        with pytest.raises(BadRequestError):
            anthropic_client.messages.create(
                model=anthropic_chat_model,
    ...
    def test_server_tool_unsupported_on_non_claude_model(
        self, anthropic_client: Anthropic, use_official_api: bool, chat_model: str
    ) -> None:
        """Test that Claude server tools raise error on non-Claude models.

        Validates:
            - bash tool on a non-Claude model raises BadRequestError
        """
        if use_official_api:
            pytest.skip("Model not supported on official API")
        with pytest.raises(BadRequestError):
            anthropic_client.messages.create(
                model=chat_model,
                max_tokens=300,
    ...
    def test_server_tool_with_custom_tool(
        ...
        """Test that Claude models accept server tools alongside custom tools.

        Validates:
            - Mixing server tools with custom tools is accepted on Claude models
            - Response is valid
        """
        response = anthropic_client.messages.create(
            model=anthropic_chat_model,
...
class TestAnthropicWebSearch:
    """Test suite for web search system tool support via the Anthropic Messages API.

    Tests validate that the ``web_search`` tool is correctly mapped to the
    Bedrock ``nova_grounding`` system tool on supported models (Amazon Nova
    Premier) and that unsupported models reject the tool with an error.
    """

    #: Nova Premier model identifier used for web search tests.
    NOVA_PREMIER_MODEL = "amazon.nova-premier-v1:0"

    @pytest.fixture(autouse=True)
    def _skip_non_local(self, use_anthropic_api: bool) -> None:
        """Skip web search tests when running against the official Anthropic API."""
        if use_anthropic_api:
            pytest.skip(
                "Web search system tool tests require a local server with Nova Premier"
            )

    def test_web_search_basic(self, anthropic_client: Anthropic) -> None:
        """Test basic web search tool invocation on Nova Premier.

        Validates:
            - Response is a valid message
            - Response contains content blocks
            - The model uses the web search system tool
        """
        response = anthropic_client.messages.create(
    ...
    def test_web_search_streaming(self, anthropic_client: Anthropic) -> None:
        """Test streaming with web search tool on Nova Premier.

        Validates:
            - Streaming completes without error
    ...
    def test_web_search_unsupported_model_raises_error(
        self, anthropic_client: Anthropic
    ) -> None:
        """Test that web search on a model without system tool support raises an error.

        Validates:
            - BadRequestError is raised for models that don't support web_search
        """
        with pytest.raises(BadRequestError):
            anthropic_client.messages.create(
