...
class TestTokens:
    ...
    async def test_a_rollover_during_the_admission_write_keeps_the_counts_whole(
        self, limited_backend: SSMClient, clock: _Clock, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """A request admitted across the boundary counts on the ledger that holds it.

        The first request of a window waits for the counter write, and any
        other request reaching the key while it waits builds the next
        window's ledger from the in-flight counts as they stood before the
        waiter added itself. Counting the waiter on the ledger it entered
        with would strand its estimate on an orphan while its release lands
        on the live ledger, and every later rollover carries that drift
        forward: the key would be admitted past its ceiling for as long as
        the instance stays warm.

        Ref: stdapi/tenant_rate_limits.py:admit_tenant_request
             stdapi/tenant_rate_limits.py:_current_ledger
        """
        key_id = "r" + "0" * 15
        key = await _declare_and_mint(limited_backend, key_id, tokens_per_minute=8000)
        arrived, released = Event(), Event()
        ...
        first, second = _request_for(), _request_for()
        waiting = create_task(
            authenticate(credentials=None, x_api_key=key, request=first)
        )
        await arrived.wait()
        # The minute turns while the first request hangs on its write, and the
        # second one rolls the ledger over before the first has counted itself.
        arrived.clear()
        clock.advance(WINDOW_SECONDS)
        crossing = create_task(
            authenticate(credentials=None, x_api_key=key, request=second)
        )
        await arrived.wait()
        released.set()
        await gather(waiting, crossing)
        await settle_tenant_reservation(first)
        await settle_tenant_reservation(second)
        await _settle(key_id)

        ledger = tenant_rate_limits._LEDGERS[key_id]  # noqa: SLF001
        assert ledger.window == _current_window()
        assert ledger.inflight_estimate == 0
        assert ledger.inflight_requests == 0
        clock.advance(WINDOW_SECONDS)
        share = tenant_rate_limits._PRIOR_DIVISOR  # noqa: SLF001
        assert (await _admissions(key, share + 2, concurrent=False)).count(0) == share
        await _settle(key_id)

    async def test_a_limit_change_during_the_admission_write_keeps_the_counts_whole(
        self, limited_backend: SSMClient, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """A limit changing under a waiting request counts it on the rebuilt ledger.

        The ledger is rebuilt whenever the limits in force change, not only
        at the minute boundary, so a record edited while the window's first
        request waits for its write orphans that request's ledger exactly as
        a rollover does.
        ...
        del limited_backend
        key_id = "r" + "1" * 15
        wide = Tenant(key_id=key_id, name="wide", tokens_per_minute=8000)
        narrow = Tenant(key_id=key_id, name="narrow", tokens_per_minute=4000)
        arrived, released = Event(), Event()
        ...
        monkeypatch.setattr(tenant_rate_limits, "add_to_item", _stalled)
        first, second = _request_for(), _request_for()
        waiting = create_task(admit_tenant_request(first.scope, wide))
        await arrived.wait()
        arrived.clear()
        narrowed = create_task(admit_tenant_request(second.scope, narrow))
        await arrived.wait()
        released.set()
        await gather(waiting, narrowed)
        await settle_tenant_reservation(first)
        await settle_tenant_reservation(second)
        await _settle(key_id)

        ledger = tenant_rate_limits._LEDGERS[key_id]  # noqa: SLF001
        assert ledger.tokens_limit == 4000
        assert ledger.inflight_estimate == 0
        assert ledger.inflight_requests == 0
        later: list[int] = []
        share = tenant_rate_limits._PRIOR_DIVISOR  # noqa: SLF001
        for _ in range(share + 2):
            try:
                await admit_tenant_request(_scope("/v1/chat/completions"), narrow)
            except ApiError as error:
                later.append(error.status)
            else:
                later.append(0)
        await _settle(key_id)

        assert later.count(0) == share
