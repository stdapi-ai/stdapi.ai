...
def ollama_client(
    ...
    The gateway mounts this dialect under ``SETTINGS.ollama_routes_prefix``
    (e.g. ``/ollama``), exactly as an operator points ``OLLAMA_HOST`` at
    ``https://your-host/ollama``, so both non-cloud hosts carry that prefix;
    Ollama Cloud has no such prefix and is left untouched.
    ...
    if test_client is not None:
        ...
        yield ollama.Client(
            host=f"{str(test_client.base_url).rstrip('/')}{SETTINGS.ollama_routes_prefix}",
...
def ollama_http(ollama_client: ollama.Client) -> httpx.Client:
    ...
    Its base URL already carries the routes prefix, so a call site passes a
    bare suffix (e.g. ``"/api/tags"``), not one built with ``ollama_route()``.
