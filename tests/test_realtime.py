...
class TestServerVadItemIdentifiers:
    ...
    def _speech_turns(
        ...
        *,
        ...
        Args:
            app_client: The pre-authenticated test client.
            fake_backend: The fake model whose script the turns are replayed from.
            transcribe: Whether the client asks for the transcripts.
            transcripts: Whether the backend reports one per turn; without them
                nothing but the end of the speech can close a turn.

        Returns:
            Every event the two turns produced, in order.
        """
        fake_backend.script = [
            SpeechStarted(0),
            SpeechStopped(100),
            *([InputTranscript("one")] if transcripts else []),
            SpeechStarted(200),
            SpeechStopped(300),
            *([InputTranscript("two")] if transcripts else []),
        ]
        terminal = (
            "conversation.item.input_audio_transcription.completed"
            if transcribe
    ...
    def test_an_untranscribed_turn_does_not_lend_its_item_to_the_next_one(
        ...
        The backend reports no transcript here: a turn it could make nothing of
        leaves the end of the speech as the only event able to close it, and a
        turn left open is one the next ``speech_started`` reuses the item of.
        """
        events = self._speech_turns(
            app_client, fake_backend, transcribe=False, transcripts=False
        )
...
class TestFunctionTools:
    ...
    def test_the_answer_to_a_call_is_checked_before_it_reaches_the_model(
        self,
        app_client: TestClient,
        fake_backend: type[_FakeModel],
        monkeypatch: pytest.MonkeyPatch,
    ) -> None:
        """A tool result is client-written text, so the guardrail reads it too.

        Whatever the client returns for a call is carried into the model's own
        context, which is the reading the guardrail is configured for: what
        reaches the model is what the check left, not what the client sent.
        """
        from stdapi import realtime  # noqa: PLC0415
        ...
        assert ("INPUT", '{"secret": 14}') in checked, checked
        assert fake_backend.opened[0].tool_results == [("call-1", '{"***": 14}')]
        assert events[-1]["item"]["output"] == '{"***": 14}', events[-1]
    ...
    def test_a_call_cancelled_while_it_is_reported_is_still_answered(
        self,
        app_client: TestClient,
        fake_backend: type[_FakeModel],
        monkeypatch: pytest.MonkeyPatch,
    ) -> None:
        """A cancellation landing mid-report leaves neither side waiting.

        Reporting a call first settles what was said before it, which is a
        check and a few sends: a barge-in lands inside that window, and the
        answer the call belongs to is over by the time it resumes. Reporting it
        then would announce a call after the ``response.done`` an SDK client
        acts on, so the client would never run the tool, and the model would
        ...
        async def _apply(text: str, *, source: str, **_: object) -> str:
            """Hold the answer's own check until the test has cancelled it."""
            if source == "OUTPUT":
                await gate.wait()
            return text
        ...
        with _connect(app_client) as websocket:
            ...
            websocket.send_json({"type": "response.create"})
            _drain(websocket, "response.output_audio_transcript.delta")
            assert gate.reached.wait(_GATE_TIMEOUT), "the call was never reported"
            websocket.send_json({"type": "response.cancel"})
            done = _drain(websocket, "response.done")[-1]
            gate.release()
            deadline = time.monotonic() + _GATE_TIMEOUT
            while not fake_backend.opened[0].tool_results:
                ...
                time.sleep(0.01)
            websocket.send_json({"type": "input_audio_buffer.clear"})
            after = _drain(websocket, "input_audio_buffer.cleared")

        assert done["response"]["status"] == "cancelled", done
        assert fake_backend.opened[0].tool_results == [
            ("call-1", _CANCELLED_TOOL_RESULT)
        ]
        assert [event["type"] for event in after] == ["input_audio_buffer.cleared"], (
            after
        )
